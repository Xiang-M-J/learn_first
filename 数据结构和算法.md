...
基本思路是从后向前，找到第一个降序排序的元素 x，然后再从最后向后向前找到第一个比 x 大的元素 y，交换 x 和 y 之后，此时原本x对应的位置之后的元素均为降序排序，需要将其转为升序排序（用两个指针交换前后元素即可）
...
def nextPermutation(nums: List[int]) -> None:
	...
	# i 之后的需要升序排列，注意在调换完之后，i之后是降序排序转换一下即可
	l = i + 1
	r = n - 1
	while l < r:
...
如果要求空间复杂度为1，并且必定存在多数元素，那么可以将多数元素看为1类，其它元素看为另一类，这样只需要对多数元素计数即可
...
如果 `arr[mid]>arr[r]` 证明右半部分出现了乱序，那么最小值在右半部分，设置 `l=mid+1`，反之左半部分，为了避免mid指向的就是最小值所以设置 `h = mid`
...
对于两个字符串 txt 和 pat，从前到后，如果`txt[i+1]=pat[j+1]`，那么考虑 `i+1` 之后的 txt 和 `j+1` 之后pat 以及 `i+1` 之后的 txt 和 `j` 之后的 pat
...
int palPartition(string& s) {
    ...
    // dp[i][j] = s[i..j] 被切分为子串均为回文字符串的最小切分次数
    int dp[n][n];
...
def minCut(s: str) -> int:
	...
	isPalin = [[True for _ in range(n)] for _ in range(n)]  # 先初始化为True
	...
	for i in range(1, n):
		if isPalin[0][i]:
			...
			for j in range(1, i+1):
				# 如果 s[j...i] 为回文字符串，只需要在s[0...j-1]的基础上加一次分割即可
				if isPalin[j][i]:  
...
在push的时候用一个栈保存目前最小的值，在pop时如果pop的值和最小栈中的栈顶元素相等时，需要也pop最小栈。
...
解决方法：将每位数字压入栈中，如果当前数字小于栈顶数字就从栈顶移除一个数字
...
3、在遍历时，大概率会先遍历到尾部，到尾部的指针指向另一个链表的头节点，这样当两个指针都重新指向另一个链表的时候，两个指针距离尾部的距离相等
...
def rearrange(node: Node):
    ...
    last_half = reverse(slow.next)
    slow.next = None   # 注意将其后序置为None
    head = node
    while head is not None:
        temp = head.next
        if last_half is not None:
            head.next = last_half
...
先用一个指针移动N次，之后另一个指针指向头节点，两个指针同时移动，直到先走的指针到达尾部，此时两个指针都走了 K-N 步，后走的指针距离尾部正好N个节点
...
采用递归方法，分别在左子树和右子树中寻找p和q，如果左右子树都能找到，那么该节点就是最近公共祖先，如果在左子树找到p或q，右子树没有找到，返回的结果就是p或q，即p或q自己就是最近公共祖先
...
该问题为多源BFS，定义一个dist矩阵，遍历原矩阵将所有的0塞入队列中，并设置`dist[i][j]` 为0，然后进行多源的BFS，对于所有的 0 遍历上下左右四个方向，更新 1 的值
...
该问题也能通过动态规划求解，不过需要需要规划两次，分别为水平向左和向上运动，向右和向下运动
...
该问题直接通过求出所有入度为 0 的点即可，入度为0的点的集合即为到达所有点的最少点集合。
...
如果对于第 `i` 个字符串可以放下，则分成选择该物品或者不选择该字符串，有 `dp[i][j][k] = max(dp[i-1][j][k], dp[i-1][j-zero][k-ones]+1`
...
def solve_s(s):
    ...
    n = len(s)
    stack = []
    stack.append(-1)     # 处理第一个括号为左括号时的情况
...
def solve(nums):
    ...
    for i in range(1, n):
        if nums[i] >= stack[-1]:
            ...
            lb = bisect_right(stack, nums[i])  # 找到更大的位置
            stack[lb] = nums[i]
    return len(stack)
```
...
def beautifulSubsets(self, nums: List[int], k: int) -> int:
	...
	def dfs(idx):
		...
		dfs(idx+1) # 不选择当前元素
		# 当前元素可选时
		if cnt.get(nums[idx] - k, 0) == 0 and cnt.get(nums[idx]+k, 0) == 0:
