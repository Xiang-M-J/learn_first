...
使用位运算判断两个单词是否包含相同字符。先对单词进行预处理，一个字母映射到二进制的一位上，判断是否包含相同元素可以通过与操作来完成
...
3、`d[i]` 与 `d[i-5]` 相交，此时有 `d[i]+d[i-4] >= d[i-2]`，同时 `d[i-1]<=d[i-3]`，同时 `d[i-2] > d[i-4]`（不满足这个条件会变成第一种情况），同时 `d[i-1]+d[i-5] >= d[i-3]`
...
大于 7 的奇数可以分解为三个质数之和，由于要求最小修建的车站数，所以对于奇数可以将其先减去2，再判断是否为质数，如果是，需要两个车站，如果不是需要三个车站
...
>python 中可以直接对负数取余数得到[0, k) 范围内的数字，而对于 C++等语言来说需要加上在取余的结果后面加上 k
...
该问题可以转为找到一个最小的子序列长度使得其之和大于k，可以使用滑动窗口解决，那么该长度减1，便是最大的序列长度，使得所有小于该长度的子序列之和小于k。
...
def shortestPalindrome(s: str) -> str:
	...
	for i in range(n):
		left = (left * base + ord(s[i])) % mod    # 分别从两边开始算
...
def removeDuplicateLetters(s: str):
	...
	stack = []
	for c in s:
		if not vis[ord(c) - ord('a')]:  # 如果元素还未入栈
...
这里需要使用快慢指针找到中点，不过这里需要注意一下，需要找到中点的前一个节点，因此快慢指针有需要特殊的写法
...
类似的还有 [452. 用最少数量的箭引爆气球 - 力扣（LeetCode）](https://leetcode.cn/problems/minimum-number-of-arrows-to-burst-balloons/description/)，先按照右端点排序，先对第一个气球的右端点射出一只箭，计算这只箭能够覆盖的区间范围。如果不能覆盖，则再对着右端点射出一只箭。
