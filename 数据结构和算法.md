...
另一种方法为直接搜索h-index，由h-index的定义可知，h-index最大为n，将左右设置为 `1` 和 `n`，数组访问时为 `mid-1`，这样不会越界，但是要初始设置结果为0，这样可以返回0这个答案
