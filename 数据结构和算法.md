...
每个数都可以表示为 `10a+b` 的形式，已知 `7a + 7b` 能被 `7` 整除，那么只需要判断 `10a+b-7a-7b` 即 `3a-6b` 即 `a-2b` 能否被 `7` 整除。
...
如果去模拟蚂蚁移动的过程会很麻烦，可以这样思考当两个蚂蚁相遇时它们改变方向移动，其实就相当于两个蚂蚁穿过彼此移动，如 a 和 b 相遇，b 后面要走的路实际上是 a 要走的路，类似地，a 后面要走的路实际上是 b 要走的路
...
令 `a*x + b*y = target` 有解的条件是 target 是 x 和 y 的最大公约数的倍数（贝祖定理）
...
下面再来看一种解法，用一个队列保存当前水量，如果出现了目标水量证明有解
...
峰值元素是指其值严格大于左右相邻值的元素。给定一个元素全部都不相等的整数数组 `nums`，找到峰值元素并返回其索引。数组可能包含多个峰值，在这种情况下，返回 **任何一个峰值** 所在位置即可。可以假设 `nums[-1] = nums[n] = -∞` 。

下面介绍一个时间复杂度为 $\log(n)$ 的解法
...
如果 `nums[i−1]>nums[i]<nums[i+1]`，那么位置 i 位于山谷，两侧都是上坡，我们可以朝任意方向走。这种情况可以设置向两边更大值的方向走

在上面的四种情况下，如果 `nums[i] < nums[i+1]`，那么永远不会向左走，如果 `nums[i]>nums[i+1]`，那么永远不会向右走，因此可以用二分查找的方法
...
+ 如果 `nums[i+1]-num[i] = d`，那么又会多出一个等差数列，$t_{i+1}=t_i+1$
...
Python 的 bisect 包提供了 `bisect_left` 和 `bisect_right`，其中 `bisect_left` 是找到大于等于 `x` 的第一个位置（偏左），`bisect_right` 是找到大于 `x` 的位置（偏右）。
...
快速选择算法基于快速排序算法，partition的部分一致，只是在后续搜索时有差别，当找到 pivot 的位置时，比较该位置左侧是否包含了k-1个数，如果包含了，那么 pivot 就是需要的数，如果左侧大于 k-1 个数，就在左侧找，否则在右侧找。
...
给定一个升序排列的整数数组，找到一个 n 使得 n 个数字大于等于 n，使用二分查找解决这个问题

由于本题的实际范围为 `[0, n]`，为了避免数组越界，将左右设置为 `0` 和 `n-1`，最终的答案为 `n-mid`
...
def hIndex(citations) -> int:
	...
	while left <= right:
		...
		if citations[mid] >= n-mid: # 如果mid处的引用数大于n-mid，证明结果至少为n-mid
			right = mid - 1
			ans = n - mid
		else:
			left = mid + 1
...
另一种方法为直接搜索h-index，由h-index的定义可知，h-index最大为n，将左右设置为 `1` 和 `n`，数组访问时为 `n-mid`，这样不会越界，但是要初始设置结果为0，这样可以返回0这个答案
...
如果 `nums[mid] > nums[r]` 证明右半部分出现了乱序，那么最小值在右半部分，设置 `l=mid+1`，反之在左半部分
...
def findMin(nums):
	...
	while left <= right:
		...
		if nums[mid] > nums[right]:  # 右边出现乱序，最小值在右边
			left = mid + 1
		else:
			ans = min(ans, nums[mid])  # 更新最小值
			right = mid - 1
	return ans
```
...
[220. 存在重复元素 III - 力扣（LeetCode）](https://leetcode.cn/problems/contains-duplicate-iii/)
...
该问题可以使用一个窗口限制indexDiff内的元素，然后用二分搜索搜索到比 `nums[i]-valueDiff` 大的元素的位置，并且判断该元素 x 是否满足 `x <= nums[i]+valueDiff`。因此需要使用bisect提供的insort方法生成有序列表

```python
def containsNearbyAlmostDuplicate(self, nums, indexDiff: int, valueDiff):
	window = []
	...
	for i in range(len(nums)):
		...
		if len(window) != 0:
			idx = bisect.bisect_left(window, nums[i]-valueDiff)
...
还可以利用桶排序来做，设置桶的大小为 `valueDiff+1`，如果两个元素处于同一个桶内那么肯定满足条件，如果不在同一个桶中，那就判断左右两个桶是否存在满足条件的元素。这里需要注意的是，每个桶只要保留一个数字即可，因为只要一个桶内有两个数字，那就可以直接返回True了。

这里计算桶的位置时需要注意，如果使用python中的整除 `//` 就无需考虑正负。（如果是 C/C++，那么需要考虑负数时的情形，负数需要先加1，假设 w = 10，那么0-9会被分为一组，-1到-10应该被分为一组，如果负数不先增加1，那么-10会被分到另一组。）

```python
def containsNearbyAlmostDuplicate(nums, indexDiff: int, valueDiff: int):
	mp = {}

	# 将元素分到对应的桶内
	def getId(num, w):
		return num // w
		# else: return ((num + 1) / w) - 1  # 对于 C/C++ 等语言
	
	w = valueDiff + 1  # 桶的大小

	for i, n in enumerate(nums):
		id = getId(n, w)
		if id in mp: return True
		if id + 1 in mp and abs(n - mp[id+1]) <= valueDiff:
			return True
		if id - 1 in mp and abs(n - mp[id-1]) <= valueDiff:
			return True
		mp[id] = n
		if i >= indexDiff:
			del mp[getId(nums[i-indexDiff], w)]
...
| 排序算法 | 最好时间复杂度    | 平均时间复杂度            | 最坏时间复杂度    | 空间复杂度    | 稳定性 | 备注                 |
| ---- | ---------- | ------------------ | ---------- | -------- | --- | ------------------ |
| 冒泡排序 | O(n)       | O(n²)              | O(n²)      | O(1)     | ✅   | 最好情况是已排序           |
| 选择排序 | O(n²)      | O(n²)              | O(n²)      | O(1)     | ❌   | 每轮选择最小值            |
| 插入排序 | O(n)       | O(n²)              | O(n²)      | O(1)     | ✅   | 部分有序效果好            |
| 希尔排序 | O(n log n) | O(n^1.3) ~O(n^1.5) | O(n²)      | O(1)     | ❌   | 不稳定的插入排序改进         |
| 归并排序 | O(n log n) | O(n log n)         | O(n log n) | O(n)     | ✅   | 需要额外空间，适合链表        |
| 快速排序 | O(n log n) | O(n log n)         | O(n²)      | O(log n) | ❌   | 最常用，递归优化可避免最坏情况    |
| 堆排序  | O(n log n) | O(n log n)         | O(n log n) | O(1)     | ❌   | 基于堆结构的选择排序         |
| 计数排序 | O(n + k)   | O(n + k)           | O(n + k)   | O(k)     | ✅   | 适用于整数、小范围数据        |
| 桶排序  | O(n + k)   | O(n + k)           | O(n²)      | O(n + k) | ✅   | 分布均匀效果好            |
| 基数排序 | O(nk)      | O(nk)              | O(nk)      | O(n + k) | ✅   | k 是最大位数，适合定长整数或字符串 |
	...
	for i in range(n):
		if nums[i] == 1:
			...
			if p0 < p1:  # 将 1 交换出去了，现在要交换回来
				nums[i], nums[p1] = nums[p1], nums[i]  # nums[i] 为交换出去的1
			p0 += 1
			p1 += 1
```


另外一种方法考虑用两个指针 `p0` 和 `p2` 分别指向 0 和 2 对应的位置，从左到右遍历数组，如果遇到 2，就将其交换到 `p2` 对应的位置，同时 `p2+1`，但是此时交换过去的仍然可能是2 或者 0，因此需要连续交换直到不为2，然后判断是否为0，如果是0，将其交换到 `p0` 对应的位置
...
[969. 煎饼排序 - 力扣（LeetCode）](https://leetcode.cn/problems/pancake-sorting/description/)
...
注意求解该问题的最少翻转操作是NP难问题，但是如果只是求一个可行的翻转操作不是太难

可以采用递归的方法，首先先将最大的元素翻转到最底下，然后问题就变成了n-1的问题。将最大的元素翻转到最底下有两次操作：第一次翻转最大元素对应的下标位置，将最大元素放在最上面，第二次翻转将最大元素放在已有的最大元素之上即可。
...
如果要求排序为 `arr[0] > arr[1] < arr[2] > arr[3] < arr[4]` 这种形式[324. 摆动排序 II - 力扣（LeetCode）](https://leetcode.cn/problems/wiggle-sort-ii/)

那么上面的算法不可行。一种算法为先排序，然后不能相邻位置交换，需要将排好序的数组分成前后两部分，然后复制到原数组中

```python
def wiggleSort(nums: List[int]):

	n = len(nums)
	arr = sorted(nums)  # 对数组进行排序
...
允许对数组中的 n-1 个数加1，求使所有元素相等时的最小操作次数

直接求不好求，时间复杂度会很高，但是如果换一个思路，对 n-1 个数加 1，可以转换为对 1 个数减 1，这样就变成了每次操作对一个数减 1，需要多少次操作，数组中的元素全部变为最小值
...
假设有若干列火车到达某个站点，到达时间为 arr，离站时间为 dep，请问该站点最少需要多少个平台
...
一个未排序的元素处于一个环中，找出这个环，环的大小减一就是这个环内元素需要交换的次数。先记录未排序数组中每个元素的位置，然后对数组进行排序。遍历排序好的数组，如果已经访问过或者 `j=pos[arr[j]]`，就无需计算环的大小；否则如果 j 未被访问，则 `j=pos[arr[j]]`。
...
可以使用堆排序，维持一个大小为 k 的堆，将前 k 个元素放进堆里，如果第 k+1 个元素小于堆顶元素，那么将其放到第一个位置，否则将堆顶元素弹出，并放入第一个位置，再将第 k+1 个元素塞进去

因为每个元素最多位于正确位置距离为 k 的位置，因此第 k+2 个元素不可能会比第 1 个元素还小，否则距离就是 k+1 了。
...
如果需要设计一个时间复杂度为 `O(n)` 的算法，可以使用桶排序，由于本问题针对整数数组，因此桶的大小取 1 即可。
...
[Merge Two Sorted Arrays Without Extra Space | GeeksforGeeks](https://www.geeksforgeeks.org/merge-two-sorted-arrays-o1-extra-space/)

给定两个有序列表，将这两个列表变成一个整体有序的列表，即一个列表只包含较小的部分值，另一个列表包含较大的部分值

使用两个指针分别指向一个列表的结尾和另一个列表的开始，如果`a[i]>b[j]`，就交换，并移动到下一位，否则让一个指针继续前进，直至停止迭代。当然这样处理之后，最后还需要对两个数组分别排序。
...
def numDistinct(txt: str, pat: str) -> int:
	...
	for i in range(1, n1):
		for j in range(1, n2):
			if txt[i] == pat[j]:
			# dp[i-1][j]表示不匹配txt[i]，dp[i-1][j-1] 表示匹配 txt[i]
...
一种简单的方法：计算前缀和并将其排成有序的列表，然后使用二分查找来搜索符合条件的前缀和。
...
def countRangeSum(nums: List[int], lower: int, upper: int) -> int:
	...
	for n in nums:
		s += n
		idx1 = bisect.bisect_right(prefixs, s - lower)
		idx2 = bisect.bisect_left(prefixs, s - upper)
		cnt += idx1 - idx2
		bisect.insort(prefixs, s)
	return cnt
```

>虽然插入的时间复杂度为 O(n)，但是现代处理器对其进行了很好的优化，可以满足时间要求


另一种方法为归并排序，对数组的前缀和进行归并排序，在合并时，两个前缀和子序列 $n_1$ 和 $n_2$ 都是有序的，这样可以考虑 $n_1$ 的第一个元素，然后用两个指针找到 $n_2$ 中符合条件的前缀和范围，考虑 $n_1$ 的第二个元素，由于是递增的，所以两个指针只能向右移动
...
def countRangeSum(nums: List[int], lower: int, upper: int) -> int:
	...
	def split(left, right):
		if left >= right:
			...
			mid = left + (right - left) // 2
			n1 = split(left, mid)    
			n2 = split(mid+1, right)
			cnt = n1 + n2

			i = left  # 首先选择左边的第一个元素
			l, r = mid + 1, mid + 1  # 选择右边的第一个元素

			# 计算符合条件的区间数
			while i <= mid:
				while l <= right and prefixs[l] - prefixs[i] < lower:
					l += 1
				while r <= right and prefixs[r] - prefixs[i] <= upper:
					r += 1
				cnt += (r - l)
				i += 1
			...
			while i <= mid and j <= right:
				if prefixs[i] < prefixs[j]:
					merged.append(prefixs[i])
					i += 1
				else:
					merged.append(prefixs[j])
					j += 1
			while i <= mid:
				merged.append(prefixs[i])
				i += 1
			...
			for i in range(len(merged)):
				prefixs[left+i] = merged[i]
			return cnt
	
	cnt = split(0, len(prefixs)-1)
...
def maxWater(arr):
	...
	if len(left_arr) > 1:
		left_units = findMaxWater(left_arr)
	
	if len(right_arr) > 1:
		right_units = findMaxWater(right_arr)
...
这种做法可以使用两个指针保存左侧最大值和右侧最大值来优化空间，这里直接通过比较 `height[left]` 和 `height[right]` 来判断 `leftMax` 和 `rightMax` 谁更大（直接比较 `leftMax` 和 `rightMax` 似乎也可以），依据为每次都是移动较小的一边，因此如果 `height[left]` < `height[right]`，那么left左侧没有比`height[right]` 更大的，因此 `leftMax < rightMax`
...
def maxWater(arr):
    ...
    for i in range(len(arr)):
        while st and arr[st[-1]] < arr[i]:
            ...
            # arr[i] 是下一个更大的值，并且新的栈顶元素为前一个更大的值
            distance = i - st[-1] - 1
...
用一个最大堆保存当前窗口的元素和索引，为了弹出最大值，需要对元素取负值。删除元素时，需要注意用一个 while 判断栈顶元素的索引是否还在窗口内，如果不在窗口中，就删除栈顶元素。
...
还可以用一个双向队列来保存，队首元素为当前窗口最大的值，如果新的值比队尾元素大，就弹出队尾元素（需要用while）
...
在求最小子串时，如果同时移动左右指针，会很麻烦，不如先确定右边界，如果窗口内包含了所有的元素再调整左边界。
...
kadane算法：先计算每个索引前的最大和，然后用一个大小为 k 的窗口，计算连续 k 个元素之和，然后加上前面 i - k 个元素的最大和看是否更大。
...
给定一个字符串 `s` 和一个整数 `k` 。你可以选择字符串中的任一字符，并将其更改为任何其他大写英文字符。该操作最多可执行 `k` 次。在执行上述操作后，返回包含相同字母的最长子字符串的长度。

解决方法为用一个滑动窗口找到一个序列包含了一个序列，该序列中包括了一些重复的数据，和一些不同的数据，如果这些不同数据的数量小于 k，那么还可以继续，如果已经大于k了，就移动左边界。这里需要维护一个当前窗口中最多元素对应的数量 maxv，当移动左边界时，此时无需更新 maxv，因为如果右边界的元素正好是左边界的元素，那么无需更新，如果左边界移除以后，使得此时 maxv 的值变小，右边界如果读到了不是刚才移出左边界的字符，新的子串要想在符合题意的条件下变得更长，maxv 一定要比之前的值还要更多，因此不会错过更优的解。
...
前缀和：`prefixSum[i] = arr[0]+arr[1]+...+arr[i]`

从 L 到 R 的和用前缀和表达为（L 和 R 均从 1 开始计数）如果 L 大于1，`prefixSum[R] - prefixSum[L-1]`，否则为 `prefixSum[R]`
...
给定一个正整数序列，找到一个最大的序列长度，使得所有小于该长度的子序列之和小于k。

该题可以用二分搜索，搜索一个长度，计算所有该长度的子序列之和（使用滑动窗口）的最大值，如果这个最大值小于等于k，那么调整下界，否则调整上界。

另一种方法直接使用滑动窗口，该问题可以转为找到一个最小的子序列长度使得其之和大于k，可以使用滑动窗口解决，那么该长度减1，便是最大的序列长度，使得所有小于该长度的子序列之和小于k。
...
def solve(arr, k):
    ...
    for r in range(n):
        ...
        while s > k:
            ...
            # 对于全部大于 0 的序列来说，如果 s 等于0，意味着 l > r，
...
[Maximum subarray sum modulo m | GeeksforGeeks](https://www.geeksforgeeks.org/maximum-subarray-sum-modulo-m/)

对于一个序列，求其子序列模m后的余数的最大值

首先求前缀和关于m的余数，并存入一个集合中，每次求出余数后，在这个集合中找到比这个余数至少大1的余数，假设找到余数正好大 `a`，这样两个前缀和相减之后，余数为`m-a`。为了节约时间，使用二分搜索，同时为了保证前缀和有序，需要在插入使用二分插入。
...
def maxSub(self, arr, m):
	...
	for i in range(n):
		...
		it = 0
		# 找到大于等于prefix+1的最小元素的位置
		it_index = bisect_left(S, prefix+1)
		if it_index < len(S):
			it = S[it_index]
...
给定一个数组，计算数组中的连续区间之和可以被 M 整除的数量


使用前缀和，需要边计算前缀和，边计算区间数

```python
for i in range(N):   # 先对数组进行取余
    A[i] %= M
tot = 0
freq = defaultdict(int)   # 计算每个余数的出现次数
freq[0] = 1     # 0 这个余数的初始出现次数为1（即空数组之和为0）
cnt = 0
for i in range(N):
    tot += A[i]
    tot %= M     # 计算当前和的余数
    
    for j in range(M):    # 判断之前的前缀和是否存在满足要求的余数
        # 如果之前和当前的前缀和之差 tot - j 能够被M整除
        # 为了避免变成负数，在 tot - j加上M
        if (tot + M - j) % M == 0:  
            cnt += freq[j]
...
class NumMatrix:
    ...
    def __init__(self, matrix: List[List[int]]):
        m = len(matrix)
        n = len(matrix[0])
        self.mp = [[0 for _ in range(n+1)] for _ in range(m+1)]

        for i in range(m):
            for j in range(n):
                self.mp[i+1][j+1] = self.mp[i][j+1] + self.mp[i+1][j] \
                    - self.mp[i][j] + matrix[i][j]

    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        return self.mp[row2+1][col2+1] + self.mp[row1][col1] \
            - self.mp[row2+1][col1] - self.mp[row1][col2+1]
```
...
def isInterleave(s1: str, s2: str, s3: str) -> bool:
	...
	def dfs(i, j):
		...
		a = i < n1 and s3[i+j] == s1[i] and dfs(i+1, j)
		b = j < n2 and s3[i+j] == s2[j] and dfs(i, j+1)

		memo[i][j] = a or b
		return memo[i][j]
	
	memo = [[None for _ in range(n2+1)] for _ in range(n1+1)]
...
令 `dp[i][j]` 为 `s1[:i]` 和 `s2[:j]` 是否为 `s3[:i+j]` 的插值

边界条件：`dp[0][0]` 为True

分成两个条件：如果 `s1[i-1]` 和 `s3[i+j-1]` 相等，则 `dp[i][j] |= dp[i-1][j]`

如果 `s2[j-1]` 和 `s3[i+j-1]` 相等，则 `dp[i][j] |= dp[i][j-1]`
...
采用记忆化搜索的方法，对于一个位置，可以选择将其作为开始，或者不选择其作为开始。如果选择作为开始，则遍历长度大于等于 k 的字符串末端，如果找到一个可以形成回文字符串，就停止该位置的搜索。
...
def solve(s):
    ...
    def _solve(i, res):
        if i == n:
            if res != "": sols.append(res)
            return
        _solve(i+1, res)  # 选择该字符
        _solve(i+1, res+s[i])  # 不选择该字符
...
def isScramble(s1: str, s2: str):
	...
	# dp[i][j][l]: s1[i:i+l] 和 s2[j:j+l] 是否为scramble
	...
	for l in range(2, n+1):
		for i in range(n-l+1):
			for j in range(n-l+1):
				for k in range(1, l):  # 将 [i: i+l] 分割为两部分
					# 不交换
					noswap = dp[i][j][k] and dp[i+k][j+k][l-k]
					# 交换
					swap = dp[i][j+l-k][k] and dp[i+k][j][l-k]
					if swap or noswap:
						dp[i][j][l] = True
						break
	return dp[0][0][n]
...
判断一句话能否被给定的词表分解，使用动态规划的方法，通过一个数组保存 `s[0..i]` 时是否可以被分，如果可以，就尝试一下在 `i+1` 之后是否存在可以被分解的，每次只取最短的一种可能。
...
给定一个序列，该序列由 `I` 和 `D` 这两种字符构成，`I` 表示增加，`D` 表示减小，如 `DI`可以表示为 `213`，求出给定的序列所能表示的最小数字

首先观察到如果第一个字符为 `I`，那么最开始的输出为 `12`，否则为 `21`，同时能用的数字为3。现在从第二个字符开始遍历，如果是 `I`，那么直接将还未使用的最小值加到后面即可，并且更新还未用的最小值和最近的 `I`；如果索引 i 处的字符是 `D`，那么将输出中的第 i 个元素加到输出中，并且对于前一个 `I` 和 `i` 处的结果全部加1。
...
使用动态规划的方法，在第一个字符串中添加一个元素，为了使最长公共子序列加1，那就要这个元素匹配到第二个序列的某个元素，并且删除掉第二个序列的这个元素后，仍然能够保证最长公共子序列长度不变。
...
问题的关键是如何根据排序的字符串找到对应的字符之间的顺序，一种方法是枚举字符串，比较字符串之间的差距，如果字符不同，那么就可以得到字符之间的顺序。这些顺序构成了有向边，进行拓扑排序即可
...
def solve(mat, word):
    def _solve(mat, word, x, y, idx):
        ...
        if mat[x][y] == word[idx]:
            ...
            _solve(mat, word, x-1, y, idx+1) or \
            _solve(mat, word, x, y+1, idx+1) or \
            _solve(mat, word, x, y- 1, idx+1)
...
如果需要多次搜索单词 [212. 单词搜索 II - 力扣（LeetCode）](https://leetcode.cn/problems/word-search-ii/description/)，需要用到字典树，对于每个位置进行dfs，每找到一个单词就删除一个
...
def findWords(board, words):
	...
	def dfs(i, j, now):
		# 如果当前字符不在子节点中，则证明不存在字符
		if board[i][j] not in now.children:
			return

		ch = board[i][j]
		nxt = now.children[ch]  # 遍历子节点

		if nxt.word != "":    # 如果这里包括一个单词
			ans.append(nxt.word)
			nxt.word = ""     # 删除这里的单词
		
		if nxt.children:      # 如果有子节点
			board[i][j] = "#"   # 标记已经访问过，防止重复访问
		...
		if not nxt.children:  # 去除空节点，可加可不加
			now.children.pop(ch)
...
首先添加的字符串 `s'` 的长度一定小于原本的字符串长度，因为可以去除第一个字符后反转再添加回去得到一个回文串。这样可以将 s 分为两部分，首先是长度为 $|s|-|s'|$ 的前缀 $s_1$，以及长度为 $|s'|$ 的后缀 $s_2$。由于 $s'+s$ 是一个回文串，那么 $s'$ 的反序就必然与 $s_2$ 相同。也就是说，需要找到一个最长的 $s_1$ 满足是回文串，这样剩余部分便为 $s_2$。

为了计算回文串，可以使用Rabin-Karp算法，回文字符串的哈希相同（忽略哈希碰撞的问题）
...
def shortestPalindrome(s: str) -> str:
	...
	for i in range(n):
		left = (left * base + ord(s[i])) % mod    # 从左侧算哈希值
		right = (right + mul * ord(s[i])) % mod   # 从右侧算哈希值
		if left == right:
			best = i
		mul = mul * base % mod
...
该问题可以用二分查找搜索最长长度，范围为 `[0, n-1]`。为了高效判断 s 中是否有长度为 L 的重复子串，因此可以用 Rabin-Karp 算法对固定长度的字符串进行编码。在 s 中 n−L+1 个长度为 L 的子串中，有两个子串的编码相同时，则说明存在长度为 L 的重复子串。首先对 s 的每个字符进行编码，得到一个数组 arr，将子串看成一个 26 进制的数，如 abc 编码为 $0 * 26^2 + 1 * 26 + 2 * 26^0 = 28$。当 L 很长时，需要取模，但是这样会造成哈希碰撞，为了避免碰撞，使用双哈希，即用两套进制和模的组合，来对字符串进行编码。只有两种编码都相同时，我们才认为字符串相同。

```python
def longestDupSubstring(s: str) -> str:
	# 生成两个进制和两个模
	a1, a2 = random.randint(26, 100), random.randint(26, 100)
	mod1, mod2 = random.randint(10**9+7, 2**31-1), \
	 random.randint(10**9+7, 2**31-1)
	...
	def check(arr, m):
		...
		h1, h2 = 0, 0
		for i in range(m):  # 计算字符串的哈希
			h1 = (h1 * a1 + arr[i]) % mod1
			h2 = (h2 * a2 + arr[i]) % mod2
		
		seen = {(h1, h2)}

		for ss in range(1, n-m+1):
			h1 = (h1 * a1 - arr[ss - 1] * aL1 + arr[ss+m-1]) % mod1
			h2 = (h2 * a2 - arr[ss - 1] * aL2 + arr[ss+m-1]) % mod2
...
[301. 删除无效的括号 - 力扣（LeetCode）](https://leetcode.cn/problems/remove-invalid-parentheses/description/)

给你一个由若干括号和字母组成的字符串 `s` ，删除最小数量的无效括号，使得输入的字符串有效。

有两种解法，第一种是回溯法，但是要注意，为了降低复杂度，需要先计算出需要删除的左括号和右括号的数量。左括号删除的数量为左括号的数量减去右括号的数量，右括号删除的数量则为左括号为0时，右括号的数量
...
def removeInvalidParentheses(self, s: str) -> List[str]:
	...
	def dfs(s, idx, lr, rr):
		...
		# 尝试对每个位置进行删除（因为删除后的字符串长度会变化，需要每次都进行判断）
		for i in range(idx, len(s)):
			if i != idx and s[i] == s[i-1]:  # 跳过重复的元素
				continue
			...
			# 尝试删除左括号
			if lr > 0 and s[i] == "(":
				dfs(s[:i] + s[i+1:], i, lr-1, rr)
			# 尝试删除右括号
			if rr > 0 and s[i] == ")":
				dfs(s[:i] + s[i+1:], i, lr, rr-1)
	dfs(s, 0, lremove, rremove)
	return sols
```


第二种方法，由于是删除最小数量的括号，因此可以使用BFS来解决这一问题，最简单的是尝试删除每个括号，并且用一个集合标记已经访问过的字符串，这样如果能找到合法的字符串就返回（因为这是删除最小数量的结果）
...
def lexicalOrder(self, n: int) -> List[int]:
	...
	for i in range(n):
		...
		if num * 10 <= n:
			...
			# 如果num % 10 == 9 或者 num+1>n 证明当前长度已经搜索完了
			# 退回到上一位
...
def hanota(A: List[int], B: List[int], C: List[int]) -> None:
	def hanoi(n, A, B, C):
		...
		hanoi(n-1, A, C, B) # 将 n-1 个盘子从 A 搬到 B 中
		C.append(A.pop())   # 将最后一个盘子从 A 搬到 C
		hanoi(n-1, B, A, C) # 将 n-1 个盘子从 B 搬到 C 中
	hanoi(len(A), A, B, C)
```
...
def combinationSum(candidates, target: int):
    ...
    def solve(target, idx, res):  # idx 
        ...
        for i in range(idx, n):   # 遍历所有的组合
            solve(target - candidates[i], i, res + [candidates[i]]) 
    solve(target, 0, [])
    return sols
```
...
def combinationSum(candidates, target: int):
	...
	def solve(target, idx, res):
		...
		if target >= candidates[idx]:
			solve(target - candidates[idx], idx, res+[candidates[idx]])
	solve(target, 0, [])
	return sols
```
...
先排序，在求解的时候去重，具体方法为使用一个数组标记填入的元素，如果一个元素和其前一个元素相等，但是其前一个元素还未被填入，则跳过。如果不跳过这种情况，就会出现重复的排列
...
def permuteUnique(nums):
	...
	def solve(res):
		...
		for i in range(n):
			...
			if vs[i] or (i > 0 and not vs[i-1] and nums[i] == nums[i-1]):
				continue
...
[282. 给表达式添加运算符 - 力扣（LeetCode）](https://leetcode.cn/problems/expression-add-operators/description/)

该问题需要一边回溯，一边计算

如果不在回溯内部使用循环比较麻烦，需要额外传入当前值
...
首先计算总和，如果总和不能被 k 整除，那么肯定不能划分。然后对数组进行排序，方便剪枝。由于元素数量少，这里用一个数字来表示数组中哪些元素可选。
...
def canPartitionKSubsets(nums, k: int) -> bool:
	...
	# 要求非空集合，如果最后一位大于pre，则证明无法完全分成K个集合
	if nums[-1] > pre: return False
	...
	def dfs(s, p):
		if s == 0: return True
		for i in range(n):
			# 如果 nums[i] + p 大于目标值pre，则无法分割
			if nums[i] + p > pre:
				break
			# p = (p + nums[i]) % pre，
			# 如果 p + nums[i] 小于pre，那么继续找
			# 如果 p + nums[i] 等于 pre，那么刚好置为0，找下一个集合
			if (s >> i) & 1 and dfs(s ^ (1 << i), (p + nums[i]) % pre):
				return True
		return False
	# 一开始全部位设置为1
	return dfs((1 << n) - 1, 0)
...
该问题还可以用动态规划的方法解决，还是用数字 S 表示某个元素是否被使用，总共有 $2^n$ 种状态，令 `dp[S]` 表示状态为 S 时是否可行，初始化 `dp[0]` 为 True。并且使用一个数组来表示该状态下的和，判断是否满足要求
...
def canPartitionKSubsets(nums, k: int) -> bool:
	...
	for i in range(0, 1 << n):
		if not dp[i]:  # 找到一个可行的状态
			continue
		for j in range(n):
			if cursum[i] + nums[j] > per:
				break
			if (i >> j & 1) == 0:  # 如果第 j 个元素可用
				next = i | (1 << j)  # 检查可行状态的前一个状态，将其设置为可行
				if not dp[next]:
...
该问题直接求解不好求，可以采用二分查找的方法，确定二分查找的上下界，对于查找的目标值 limit，如果存在一个方案使得所有的工人的工作时间都不超过limit，则证明该limit为可能的解。


一个朴素的方案是，开辟一个大小为 k 的数组 `workloads`，`workloads[i]` 表示第 i 个工人的当前已经被分配的工作量，然后我们利用一个递归函数 `backtrack(i)` 递归地枚举第 i 个任务的分配方案，过程中实时地更新 `workloads` 数组。具体地，函数中我们检查每一个工人 j 当前已经被分配的工作量，如果被分配的工作量 `workloads[j]` 与当前工作的工作量 `jobs[i]` 之和不超过 limit 的限制，我们即可以将该工作分配给工人 j，然后计算下一个工作 `jobs[i+1]` 的分配方案。过程中一旦我们找到了一个可行方案，我们即可以返回 true，而无需枚举完所有的方案。

```python
def minimumTimeRequired(jobs, k: int) -> int:
	def dfs(jobs, workloads, idx, limit):
		if idx >= n: return True
		curr = jobs[idx]
		for i in range(k):
			if workloads[i] + curr <= limit:
				workloads[i] += curr
				if dfs(jobs, workloads, idx+1, limit):
					return True
				workloads[i] -= curr
			# 如果当前工人未分配工作，那么下一个工人必然未被分配工作
			# workloads[i] 为0，可能是因为limit过小
			# 或者当前工作恰使该工人工作量达到上限
			# 这两种情况都无需继续尝试分配工作
			if workloads[i] == 0 or workloads[i] + curr == limit:
				break
		return False
	
	def check(jobs, k, limit):
		workloads = [0] * k   # k 个工人对应的工作量
		return dfs(jobs, workloads, 0, limit)
	
	n = len(jobs)
	ans = 10 ** 9
	jobs.sort(reverse=True)  # 从大到小分配，先分配大的更加容易
	l, r = jobs[0], sum(jobs)
...
def countRecur(coins, n, sum):
    ...
    return countRecur(coins, n, sum - coins[n - 1]) + \
              countRecur(coins, n - 1, sum)   # 不选择该硬币，还剩n-1个硬币可选
...
def get_all_subsets2(arr, l, idx, subs):
    ...
    get_all_subsets2(arr, l + [arr[idx]], idx+1, subs)
...
如果数组中存在重复元素，为了找到不重复的子集合，需要在不选择当前位置时同时不选择和其相等的所有值（需要先排序）

```python
def subsetsWithDup(nums) -> List[List[int]]:
	nums.sort()
	sols = []
	n = len(nums)
	def dfs(i, res):
		if i == n:
			sols.append(res[:])
			return
		dfs(i+1, res+[nums[i]])   # 选择当前位置的话，一切正常

		# 不选择 i+1，则跳过所有和 i+1 相等的值
		i += 1
...
def isAdditiveNumber(self, num: str) -> bool:
	...
	n = len(num)
	ans = [False]
	def dfs(idx, first, second, s):  # idx 为一个数的结尾索引，s 为一个数的开始索引
		if idx >= n:
			return
		if ans[0]:   # 减少不必要的递归
			return
		
		if first == "":  # 如果第一个数还没有确定
			if idx <= n // 2:   # 第一个数的长度不能超过一半
				dfs(idx+1, first, second, s)
			
			first = num[s:idx+1]
			if len(first) > 1 and first[0] == "0":
				return
			dfs(idx+1, first, second, idx+1)
		elif second == "":   # 如果第二个数还没有确定
			if idx <= n // 2:
				dfs(idx+1, first, second, s)
			second = num[s:idx+1]
			if len(second) > 1 and second[0] == "0":
				return
			# 第三个数的长度应该是前两个数长度较大的一个
			width = max(len(second), len(first)) 
			dfs(idx+width, first, second, idx+1)
		else:   # 确定第三个数
			third = num[s: idx + 1]
			if len(third) > 1 and third[0] == "0":
				return
			width = max(len(first), len(second))
			if width <= len(third) <= width + 1:
				if int(third) == int(first) + int(second):
					if idx == n - 1:
						ans[0] = True
					width = max(width, len(third))
					dfs(idx + width, second, third, idx+1)
				else:
					dfs(idx + 1, first, second, s)
...
[放苹果_牛客题霸_牛客网](https://www.nowcoder.com/practice/bfd8234bb5e84be0b493656e390bdebf?tpId=37&tags=&title=&difficulty=0&judgeStatus=0&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3FtpId%3D37)

将 m 个相同的苹果放入 n 个相同的盘子中，允许有的盘子空着不放。求解有多少种不同的分法。


- 递归退出条件：
    - 如果没有苹果，则说明放置已经完成，方案已定，无需计算
...
- 如果盘子数量小于等于苹果数量
    ...
    - 如果不要空这些盘子，则相当于所有盘子里至少会放一个苹果，那就相当于在每个盘子里都放一个苹果，这些苹果不影响方案数量，因此有solution(m-n,n)种方案
...
def place(m, n):
    if m < 0 or n < 0:
        return 0
    if m == 1 or n == 1:   # 如果只有一个苹果或者一个盘子，只有一种放法
        return 1
    # place(m, n-1) 当前盘子不放，place(m-n, n) 在当前的n个盘子中都放一个苹果
...
[火车进站_牛客题霸_牛客网](https://www.nowcoder.com/practice/97ba57c35e9f4749826dc3befaeae109?tpId=37&tags=&title=&difficulty=0&judgeStatus=0&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3Fpage%3D2%26tpId%3D37%26type%3D37)
...
def dfs(stack, idx, ou_stack):
    if len(stack) == 0 and idx == n:
        sol.append(" ".join(map(str, ou_stack)))
        return

    if idx < n:   # 如果还可以入栈
        dfs(stack + [arr[idx]], idx + 1, ou_stack)
    if len(stack) > 0:  # 如果还可以出栈
        p = stack.pop()
        dfs(stack, idx, ou_stack + [p])
...
def mul(self, res, mat, m):
	...
	for i in range(3):
		for j in range(3):
			for k in range(3):
				# Multiplying matrices element-wise and summing
...
def mat_exp(self, n, m):
	while n>0:
		if n&1:  
			# Multiplying the result matrix with the current matrix
...
还有一种时间复杂度更低的方法，Z 字查找，我们可以从矩阵 matrix 的右上角 `(0,n−1)` 进行搜索。在每一步的搜索过程中，如果我们位于位置 `(x,y)`，那么我们希望在以 matrix 的左下角为左下角、以 `(x,y)` 为右上角的矩阵中进行搜索，即行的范围为 `[x,m−1]`，列的范围为 `[0,y]`：
...
遇到操作符，比较操作符和目前栈顶的操作符的优先级顺序（`(`的运算级顺序设为最低），如果大于压入栈，小于等于就出栈，出栈的操作符加在结果中，遇到操作数就放入结果中，遇到 `(` 直接入栈，遇到 `)` 出栈（出栈结果加在结果中）直到遇到 `(`
...
def maxOfMins(self, arr):
        ...
        # 对于第i个元素，窗口大小为lenArr[i]时该元素为其中的一个最小值
    
        # 注意这里的windowSize减一是因为数组从0开始索引
        # 即 res[0] 为长度为1的窗口的结果
        for i in range(n):
            windowSize = lenArr[i] - 1   
            res[windowSize] = max(res[windowSize], arr[i])
    
        # 窗口长度为 i + 1 的最大最小值一定小于等于窗口为i的最大最小值
        for i in range(n - 2, -1, -1):
...
该题使用单调栈求解。首先正向遍历字符串，维护当前的结果（可以用一个栈来保存结果），默认第一个字符入栈。如果当前字符比栈末尾的字符大，那么可以加在后面；如果当前字符比栈末尾的字符小，并且栈末尾的字符后面还有（先统计字符串中的字母个数，判断该字母个数是否为0），就弹出栈末尾的字符；如果栈末尾的字符后面没有了，还是加在后面。

为了避免重复，用一个数组标记已经入栈的字符，入栈置为True，出栈置为 False
...
def removeDuplicateLetters(s: str):
	...
	stack = []
	for c in s:
		if not vis[ord(c) - ord('a')]:  # 字符未入栈
			while len(stack) and stack[-1] > c:  # 如果栈尾字符大于当前字符
				# 如果后面还有栈尾字符，弹出栈尾字符
				if cnt[ord(stack[-1]) - ord('a')] > 0:  
					...
					break
			vis[ord(c) - ord('a')] = True # 当前字符入栈
			stack.append(c)
		cnt[ord(c) - ord('a')] -= 1    # 删除当前字符的计数
	return "".join(stack)
```
...
[321. 拼接最大数 - 力扣（LeetCode）](https://leetcode.cn/problems/create-maximum-number/)
...
该问题的核心问题是要找到一个数组中的固定长度的最大子序列，然后就是找到长度为 `x` 和长度为 `k-x` 的最大子序列。遍历所有的长度组合，确定最大子序列

为了求固定长度为 k 的最大子序列，先维护一个长度为 k 的栈，以及指向最小元素的指针 top，并且需要注意是否还有剩余的元素。如果栈为空，直接添加元素，并将top指向该元素。如果栈非空，如果top元素小于当前元素，则需要考虑在有足够元素填补后续序列的基础上找到栈中小于该元素的最大元素 p，将 p 替换为该元素。

在合并序列时，也要注意当两个元素相等时需要继续向后判断大小，

```python
def maxNumber(nums1: List[int], nums2: List[int], k: int):
	n1, n2 = len(nums1), len(nums2)
	maxSubsequence = [0] * k
	start, end = max(0, k - n2), min(k, n1)
	# 计算固定长度的最大子序列
	def getMaxSubsequence(nums, k):
		if k == 0: return []
		remains = len(nums) - k  # 需要填入k个元素，所以剩余的为n-k个
		top = -1
		stack = [0] * k
		for i, num in enumerate(nums):
			# 如果最小元素小于当前元素，并且还有剩余的元素
			while top >= 0 and stack[top] < num and remains > 0:
				top -= 1
				remains -= 1   # top后的元素不会被使用，所以剩余元素减1
			
			if top < k - 1:  # 栈中的元素存在比num小的
				top += 1
				stack[top] = num  # 替换掉最小元素
			else:
				remains -= 1  # 当前元素不使用
		return stack

	def mergesequence(lss, rss):
		i, j, l1, l2 = 0, 0, len(lss), len(rss)
		if l1 == 0: return rss
		if l2 == 0: return lss
		merged = []
		while i < l1 and j < l2:
			if compare(lss, i, rss, j):
				merged.append(lss[i])
				i += 1
			else:
				merged.append(rss[j])
	...
	# 比较函数
	def compare(ss1, idx1, ss2, idx2):
		l1, l2 = len(ss1), len(ss2)
		while idx1 < l1 and idx2 < l2:
			if ss1[idx1] > ss2[idx2]:
				return True
			elif ss1[idx1] < ss2[idx2]:
				return False
			idx1 += 1
			idx2 += 1
		# 当两个序列从idx开始的子序列都相等时，哪个子序列越长就越大
		if l1 - idx1 > l2 - idx2:   
			return True
		return False

	for i in range(start, end+1):
		lss = getMaxSubsequence(nums1, i)
		rss = getMaxSubsequence(nums2, k - i)
		mss = mergesequence(lss, rss)
		if compare(mss, 0, maxSubsequence, 0):
			maxSubsequence = mss
...
使用拓扑排序解决，拓扑排序可以选择DFS或者BFS实现。

DFS解决该问题时，需要注意的是，为了防止出现环，需要加入一个栈递归判断，如果出现了环，证明不可能学完所有的课程
...
另一种方法是使用BFS，最开始能学的课程一定是没有前置课程的，即入度为0，将入度为 0 的节点全部放入队列中，取出一个节点时，删除该节点相关的所有出度，如果有节点的入度变为0，再将其加入队列，如果最后所有课程都能学习，即加入过队列的节点数量等于整个图的节点数量
...
def solve(head: ListNode, left, right):
    ...
    while i <= right-1:
        ...
        i += 1
    node.next = prev
    # 如果后面（next）还有节点，先遍历到当前的最后一个节点，再将next加到后面
    if next is not None:
...
每k个元素翻转一次链表，这里的翻转更复杂。下面以k=3为例，-1表示虚拟节点，p 表示前一个结点，c 表示当前节点，n 表示下一个结点

```
-1 -> 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7      n 指向 c 的下一个结点
 p    c    n
-1 -> 1 2 -> 3 -> 4 -> 5 -> 6 -> 7         令 c 指向 n 的下一个结点 3
 p    c n
-1 -> 1 -> 3 -> 4 -> 5 -> 6 -> 7   1 <- 2  令n指向p的下一个结点 1
p     c                            c    n
-1 -> 2 -> 1 -> 3 -> 4 -> 5 -> 6 -> 7      令p 指向 n
p     n    c

-1 -> 2 -> 1 -> 3 -> 4 -> 5 -> 6 -> 7      n 指向 c 的下一个结点
p          c    n 
-1 -> 2 -> 1 3 -> 4 -> 5 -> 6 -> 7         c 和 n 均指向 n 的下一个结点
p          c n
-1 -> 2 -> 1 -> 4 -> 5 -> 6 -> 7    3 -> 2 n 指向p 的下一个结点 2
p          c                        n     
-1 -> 3 -> 2 -> 1 -> 4 -> 5 -> 6 -> 7      p 指向 n
p     n         c

最后更新 p = c，c = c.next 即可进入下一个翻转
```
...
def reverseKGroup(head, k):
	...
	while count >= k:
		for i in range(k-1):
			...
			curr.next = next.next # 将curr的next指向下下个的结点
			next.next = prev.next # 将下一个结点指向当前k个数据的头节点
			prev.next = next      # 
...
第二种方法不直接统计两个链表的长度，而是让两个指针都经过两个链表，具体方法如下
...
计算一个完全二叉树的节点数，这个问题很容易设计一个 `O(n)` 的算法，下面介绍一个时间复杂度为 $O(\log^2(n))$ 的算法

首先先计算树的高度 h，这样可以确定树的节点数为 $2^h$ 到 $2^{h+1}-1$，然后用二分搜索的方式找到一个节点，判断该节点是否位于二叉树中。假设要判断的节点为 12，其二进制表示为 `1100`，最高位为1，之后 1 表示右子树，0表示左子树，只要不为空，证明存在
...
def countNodes(root: Optional[TreeNode]) -> int:
	...
	left, right = 2 ** h, 2 ** (h+1) - 1

	def check_exist(bstr):
		curr = root
...
使用层序遍历每一层，记录节点的位置，左节点 2 * i，右节点 2 * i+1
...
一种方法是直接用BFS，如果哪个结点没有子结点了，就返回深度。另外一种是DFS，这里需要注意不能直接返回 `min(left, right)+1`，这只在左右子树均为空或者均不为空时才正确，当一个子树为空时，要返回不为空的子树的深度
...
采用递归方法，分别在左子树和右子树中寻找 p 和 q，如果左右子树都能找到，那么该节点就是最近公共祖先，如果在左子树找到 p 或 q，右子树没有找到，返回的结果就是 p 或 q，即 p 或 q 自己就是最近公共祖先（递归的性质保证是最近的）
...
给定二叉搜索树的根节点 `root` ，该树中的恰好两个节点的值被错误地交换。请在不改变其结构的情况下，恢复这棵树。

使用中序遍历的迭代形式遍历结点，然后记录下一个值比上一个值更小时的位置
...
[814. 二叉树剪枝 - 力扣（LeetCode）](https://leetcode.cn/problems/binary-tree-pruning/description/)

给定一个只包含0和1 的二叉树，返回移除所有不包含 1 的子树的原二叉树

如果从上到下，判断每个子树是否包含1，工作量比较大，因此可以从下到上，如果为叶子节点，并且为 0 就删除。
...
这里需要使用快慢指针找到中点，不过这里需要注意一下，需要找到中点的前一个节点，因此快慢指针需要特殊的写法
...
def sortedListToBST(head):
	def dfs(head):
		...
		while fast is not None and fast.next is not None:
			fast = fast.next.next
			if fast is None:  # 这里需要提前中止，便于将中点单独拿出来
				break
			slow = slow.next
		
		next = slow.next
		slow.next = None
		root = TreeNode(next.val)
		
		root.left = dfs(head)
		root.right = dfs(next.next)
		return root
	root = dfs(head)
	return root
```
...
def buildTree(preorder: List[int], inorder: List[int]):
	inorder_pos = {}
	for i in range(len(inorder)):  # 先保存位置
	...
	def build(left, right):
		...
		root_idx = inorder_pos[root_val]

		pre_idx[0] += 1

		root = TreeNode(root_val)
		root.left = build(left, root_idx-1)    # 注意需要先构造左子树
...
同理，如果是后序遍历的结果，则根节点一开始位于最右侧，并且需要先构造右子树
...
本题实际上是求数组中哪些单词是其它单词的后缀，因此可以先将所有的单词存入集合中，再遍历每个单词的所有后缀，删除后缀对应的单词
...
def minimumLengthEncoding(words: List[str]) -> int:
	...
	for word in words:
		for k in range(1, len(word)):
			prefixs.discard(word[k:])   # discard 即使不存在也不会引发异常
	
	return sum(len(word) + 1 for word in prefixs)
```

另一种做法是利用字典树，将单词先反序存入字典树中，然后计算叶子节点对应的单词的长度
...
>如果需要找到二叉搜索树中第 k 大的数，有这样的解法，使用中序遍历的倒序获得从大到小的排列，在遍历如果发现已经找到第 k 大的数时停止后续的迭代


```python
def findTargetNode(root: Optional[TreeNode], cnt: int):
	def inorder(root):
		if root is None:
			return
...
[隐匿社交网络_牛客题霸_牛客网](https://www.nowcoder.com/practice/2870f9db6aeb4eb08fbd6460397f9bf4?tpId=37&tqId=43271&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3Fpage%3D2%26tpId%3D37%26type%3D37&difficulty=undefined&judgeStatus=undefined&tags=&title=)

将权重转为二进制，如果对应位为1，将这些合并起来
...
[336. 回文对 - 力扣（LeetCode）](https://leetcode.cn/problems/palindrome-pairs/)

给定一组字符串，求出一对字符串使其连接的结果为回文串

暴力解法会超时，可以尝试使用字典树来降低查找的时间。字典树每个节点保存了

[336. 回文对 - 力扣（LeetCode）](https://leetcode.cn/problems/palindrome-pairs/solutions/2347656/you-yu-suo-you-ti-jie-yi-ti-san-jie-qian-2sur/)

[336. 回文对 - 力扣（LeetCode）](https://leetcode.cn/problems/palindrome-pairs/solutions/45018/java-trie-yi-yu-li-jie-by-copyreadmachine/)
...
选择树中任何一个节点作为根。当选择节点 `x` 作为根节点时，设结果树的高度为 `h` 。在所有可能的树中，具有最小高度的树（即 `min(h)`）被称为 **最小高度树** 。找到所有的 **最小高度树** 并按 **任意顺序** 返回它们的根节点标签列表。

如果采用暴力解法（BFS会超时，记忆化DFS会超空间）。考虑如下的解法假设 `[x,y]` 是树中最长路径 `maxdist` 对应的端点，那么最短路径肯定在最长路径中，并且最小高度为  $\left\lceil {{{maxdist} \over 2}} \right\rceil$

因此如果最长路径为偶数，则最小高度树的根节点为 `m/2` 或者 `m/2+1`，最小高度为 `m/2`

如果最长路径为奇数，则最小高度树的根节点为 `(m+1)/2`，最小高度为 `(m-1)/2`

找到最长路径的算法为：先从任一点p出发，利用广度优先搜索或者深度优先搜索找到以p点为起点的最长路径的终点 x；以节点 x 出发，找到以 x 为起点的最长路径的终点 y，x 和 y 之间的路径为最长路径。
...
[研究red子序列的红_牛客题霸_牛客网](https://www.nowcoder.com/practice/804f995a0795419c832e1ea8e2a2aa06?tpId=37&tqId=43287&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3FtpId%3D37&difficulty=undefined&judgeStatus=undefined&tags=&title=)

两个长度为 `n` 的字符串 s，t，仅包含小写字母，下标从 1 开始。每次会进行一个操作，把 $s_i$ 和 $t_i$​ 交换，给出每一次交换后字符串 s 中的 "red" 子序列和 t 中 "red" 的子序列的数量之差。


本题需要求解的是两个字符串中的 "red" 子序列（包括 "r"，"e"，"d"，"re"，"ed"和"red"）的数量的差距。

该题通过线段树解决，线段树需要维护区间内 "red" 子序列的数量。对于 "r"、"e" 和 "d" 而言，直接是左右区间的数量相加。对于 "re"，则为左右区间的 "re" 数量之和，再加上左区间的 "r" 和右区间的 "e" 的数量相乘
...
class SegTree:
    def __init__(self, n) -> None:
        self.n = n
        # 最坏情况为满二叉树
        self.seg = [Node(0, 0) for _ in range(n * 4)]
	# 建树
    def build(self, s, node, l, r):
        ...
        if l == r:
            self.seg[node].clear()
            if s[l] == "r":
                self.seg[node].cnts[0] = 1
            elif s[l] == "e":
                self.seg[node].cnts[1] = 1
            elif s[l] == "d":
                self.seg[node].cnts[2] = 1
            return
        m = l + (r - l) // 2
        self.build(s, node * 2, l, m)
        self.build(s, node * 2 + 1, m + 1, r)
        self.merge(node)
	# 合并树
    def merge(self, node):
        left = self.seg[node * 2]
        right = self.seg[node * 2 + 1]
        self.seg[node].cnts[0] = left.cnts[0] + right.cnts[0]
        self.seg[node].cnts[1] = left.cnts[1] + right.cnts[1]
        self.seg[node].cnts[2] = left.cnts[2] + right.cnts[2]
        self.seg[node].cnts[3] = (
            left.cnts[3] + right.cnts[3] + left.cnts[0] * right.cnts[1]
        )
        self.seg[node].cnts[4] = (
            left.cnts[4] + right.cnts[4] + left.cnts[1] * right.cnts[2]
        )
        self.seg[node].cnts[5] = (
            left.cnts[5] + right.cnts[5]
            + left.cnts[0] * right.cnts[4]
            + left.cnts[3] * right.cnts[2]
        )
    # 更新树
    def update(self, node, pos, val):
        if self.seg[node].l == self.seg[node].r and self.seg[node].l == pos:
            self.seg[node].clear()
            if val == 'r': self.seg[node].cnts[0] += 1
            elif val == 'e': self.seg[node].cnts[1] += 1
            elif val == 'd': self.seg[node].cnts[2] += 1
            return
        
        m = (self.seg[node].l + self.seg[node].r) // 2
        if pos <= m:
...
tr1.build(str1, 1, 0, n-1)
tr2.build(str2, 1, 0, n-1)
...
for p in poss:
    tr1.update(1, p, str2[p])
    tr2.update(1, p, str1[p])

    str1, str2 = str1[:p] + str2[p] + str1[p+1:], str2[:p] + str1[p] + str2[p+1:]
    print(tr1.seg[1].cnts[5] - tr2.seg[1].cnts[5])
```
...
对序列中的元素倒排序，每次选择最大的元素，如果这个元素和原本的位置不同就直接赋值，但是如果相同，就选择下一个最大值，并将当前最大值重新放回去
...
该问题似乎可以通过动态规划求解，不过似乎没有必要，直接使用记忆化搜索即可。初始化一个memo矩阵，用于存储从当前点出发的最长递增路径。如果一个点还没有搜索过（`memo[i][j]=0`）尝试从每个点出发，在四个方向上计算最长递增路径。
...
该问题使用普通的dfs+访问数组似乎会超时。可以使用 Hierholzer 算法
...
顺序的思考这个问题时，会容易进入死胡同（指进入了一个结点，该结点没有连接其它结点的边，但是还没有访问完所有的边）。注意到只有入度和出度相差 1 的结点会成为死胡同结点，而这种结点是最后才访问到的，因此可以改变入栈的规则，当我们遍历完一个节点所连的所有节点后，才将该节点入栈（即逆序入栈）。
...
def findItinerary(tickets: List[List[str]]) -> List[str]:
	...
	def dfs(s):
		while adj[s]:
			dist = heapq.heappop(adj[s])
			dfs(dist)
		
		min_path.append(s)  # 先遍历结点，再将结点入栈
		
	dfs("JFK")

	return min_path[::-1]  # 逆序输出结果
```
...
+ u 不是 DFS 树的根，有一个子节点 v，使得以 v 为根的子树中没有一个节点存在与 u 的祖先节点相连的边。
...
[素数伴侣_牛客题霸_牛客网](https://www.nowcoder.com/practice/b9eae162e02f4f928eac37d7699b352e?tpId=37&tags=&title=&difficulty=0&judgeStatus=0&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3FtpId%3D37)

如果两个正整数 a 和 b 满足 a + b 是一个素数，那么 a 和 b 是素数伴侣。从给定的 n 个正整数 $\{a_1, a_2, ..., a_n\}$ 中找到最多的素数伴侣的数量。

这道题使用匈牙利算法进行二分匹配，首先需要注意到的是只有奇数才有可能是素数，因此只能奇数加偶数，因此可以将所有的数字分为奇数和偶数，利用匈牙利算法求出最大匹配。
...
# 匈牙利匹配算法
def find(x, me, evens, v):
    for i in range(len(evens)):
        if isPrime(x + evens[i]) and not v[i]:
            v[i] = True
            if me[i] == 0 or find(me[i], me, evens, v):
                me[i] = x
                return True
    return False

matcheven = [0] * len(evens)

cnt = 0

for j in range(len(odds)):
    v = [False] * len(evens)
    if find(odds[j], matcheven, evens, v):
        cnt += 1
...
已知在一张图中包含两个岛，每个岛由相连（上下左右）的 1 组成，可以转变一些 0 为 1，求出使这两座岛相邻的最小转变次数。

先记录下一座岛屿的全部点，然后逐层向外扩张，直至四周出现另一座岛，为了避免重复访问，将先记录的岛全部变为 2。
...
用 m 种颜色为图顶点涂色，要求相邻顶点不同色，采用回溯法，注意求最少颜色是一个NP完全问题
...
一个矩阵中只有 0 和 1，求所有元素到 1 的距离。

该问题为多源 BFS，定义一个 dist 矩阵，遍历原矩阵将所有的 0 塞入队列中，并设置`dist[i][j]` 为0，然后进行多源的 BFS，对于所有的 0 遍历上下左右四个方向，更新到 1 的距离
...
这个题目和常规的BFS有一些差别，由于引入了 keys 这个变量，因此需要将 keys 也加入到 visited 矩阵中，为了方便表示，将keys用二进制表示，在选择下一个位置时，如果遇到钥匙，需要额外初始化一个变量来保存keys，防止四个方向出现多个key
...
def shortestPathAllKeys(grid: List[str]) -> int:
	...
	dirs = [[1, 0], [-1, 0], [0, 1], [0, -1]]
	queue = deque()
	k = 0   # 先初始化 k 为没有获得 key 的状态
	
	# 设置 k 为全部的 key 都获取时的状态
	for i in range(m):
		for j in range(n):
			if grid[i][j] == "@":
...
该问题还有类似的变体 [130. 被围绕的区域 - 力扣（LeetCode）](https://leetcode.cn/problems/surrounded-regions/description/)

这个问题求被 `X` 围绕的 `O`，直接求不好求。可以转换为求哪些 `O` 与边界相连，即从边界的 `O` 出发能到达哪些 `O`。能被访问到的 `O` 不用变，不能被访问到的 `O` 改为 `X`
...
用染色法判断，首先随便选一个点 `i` 进行染色，然后对于邻接节点，如果未染色，将其邻接节点染成另一个颜色，如果染了和点 `i` 相同的颜色，就证明这个图不是二分图。具体可以用 DFS 或者 BFS进行遍历
...
加上一个再次选择当前物品 `profit[i]+dp[i][j-weights[i]]`
...
类似的问题还有 [435. 无重叠区间 - 力扣（LeetCode）](https://leetcode.cn/problems/non-overlapping-intervals/description/)，按照右端点排序，先选择右端点最小的区间，然后判断下一个区间是否可选，如果可选，更新右端点，统计选择的区间数。

类似的还有 [452. 用最少数量的箭引爆气球 - 力扣（LeetCode）](https://leetcode.cn/problems/minimum-number-of-arrows-to-burst-balloons/description/)，先按照右端点排序，先对第一个气球的右端点射出一只箭，计算这只箭能够覆盖的区间范围。如果不能覆盖，则再对着新的右端点射出一只箭。
...
划分一组时隙，每组时隙对应一个工作，将工作按照利润从大到小排好，遍历工作和 deadline，然后从截止日期向前搜索一个未安排工作的时隙
...
还有另一种方法，按照截至时间（从小到大）和利润（从大到小）对工作进行排序，使用最小堆保存利润最大的若干个工作
...
[330. 按要求补齐数组 - 力扣（LeetCode）](https://leetcode.cn/problems/patching-array/description/)

给定一个已排序的正整数数组 `nums` ，和一个正整数 `n` 。从 `[1, n]` 区间内选取任意个数字补充到 nums 中，使得 `[1, n]` 区间内的任何数字都可以用 nums 中某几个数字的和来表示。

请返回 _满足上述要求的最少需要补充的数字个数_ 。

对于正整数 x，如果区间 `[1, x-1]` 内的所有数字都已经被覆盖，且 `x` 在数组中，则区间 `[1, 2x-1]` 都被覆盖。

如果数字 `x` 缺失，则至少需要在数组中补充一个小于或等于 x 的数，才能覆盖到 x，否则无法覆盖到x。如果区间 `[1,x−1]` 内的所有数字都已经被覆盖，则从贪心的角度考虑，补充 x 之后即可覆盖到 x，且满足补充的数字个数最少。在补充 x 之后，区间 `[1,2x−1]` 内的所有数字都被覆盖，下一个缺失的数字一定不会小于 2x。

因此可以提出一个贪心的方案，每次找到未被数组 nums 覆盖的最小整数 x，在数组中补充 x，然后寻找下一个未被覆盖的最小整数，直至 `[1, n]` 全被覆盖


```python
def minPatches(nums: List[int], n: int) -> int:  # nums已经排好序
	patches, x = 0, 1  # 最开始搜索 1 这个最小整数
	length, index = len(nums), 0

	while x <= n:
		# 如果 nums[index] 小于等于 x，说明 [1, x-1] 的区间都可以被覆盖
		# 然后覆盖的区间变为 [1, x+nums[index]-1]，寻找 x+nums[index]
		if index < length and nums[index] <= x: 
			x += nums[index]
			index += 1
		else:  # 需要添加 x，覆盖区间从 [1, x-1] 扩展到 [1, 2x-1]，判断 2x 是否在区间内
			x <<= 1
			patches += 1
	return patches
```
...
另一种思路假设数组的前 k 个数字组成的数字范围为 `[1, total]`，添加数组第 k + 1 个数字，范围变成了 `[1, total] ∪ [nums[k], total+nums[k]]`。

如果 total 小于 `nums[k]-1`， 那么 `total+1` 就无法覆盖到，因此需要添加 `total+1`，这时覆盖范围变成了 `[1, 2*total+1]`

如果 `total >= nums[k] - 1`，那么加入数组第 k + 1个数字后，范围变成了 `[1, total+nums[k]]` 
...
def minPatches(nums: List[int], n: int) -> int:
	...
	cnt = 0
	idx = 0
	while total < n:   # 此处只需要在total<n时执行，因为当 total==n 时已经满足条件了
		if idx < len(nums) and total >= nums[idx] - 1:
			total += nums[idx]
			idx += 1
		else:
			total = 2 * total + 1
...
该题可以用类似田忌赛马的思想解决，如果 `num2` 中最大的元素小于等于 `num1` 中最大元素，那么将 `num2` 中最小的元素和 `num1` 中最大元素进行配对，如果`num2` 中最大的元素大于 `num1` 中最大元素，那么直接用最大元素和 `num1` 中最大元素配对
...
def advantageCount(nums1: List[int], nums2: List[int]) -> List[int]:
	...
	nums2.sort()
	nums1.sort()
	n1l, n1r, n2r = 0, n - 1, n-1
	i = 0
	while i < n:
		if nums1[n1r] > nums2[n2r]:
			num1 = nums1[n1r]
			num2 = nums2[n2r]
			idx = pos[num2].pop()
			res[idx] = num1
...
可以将相邻两个孩子评分更高的孩子会获得更多的糖果分成左边孩子评分更高和右边孩子评分更高。这样使用两次循环实现，先正向遍历，如果比左侧更大就在左侧元素的基础上加1，再反向遍历如果右侧+1大于目前元素，就重新赋值。
...
[767. 重构字符串 - 力扣（LeetCode）](https://leetcode.cn/problems/reorganize-string/)

给定一个字符串 `s` ，检查是否能重新排布其中的字母，使得两相邻的字符不同。

先统计字符串中的字符出现次数，如果最多的字符出现次数比剩余字符出现次数总和还多2，那么不可能使得两个相邻字符不同。至于排列可以这样做：先把最多的字符排在偶数位置，然后再按照字符遍历字符出现次数，先放偶数位置，再放奇数位置
...
def reorganizeString(s: str) -> str:
	...
	maxv = 0
	maxk = None
	for k in freq.keys():  # 获取出现次数最多的字符
		if freq[k] > maxv:
			maxv = freq[k]
			maxk = k
	n = len(s)
	if 2 * maxv - n >= 2: return "" 
	
	res = [None] * n
	i = 0
	while freq[maxk] > 0:  # 先将出现次数最多的字符放置在偶数位
		res[i] = maxk
		i += 2
		freq[maxk] -= 1
	del freq[maxk]
...
该题可以使用记忆化搜索的方法解决，下面介绍一种贪心算法，我们希望替换的结果尽可能是偶数

当 n 为偶数时，仅有唯一的方法将 `n` 替换为 `n / 2`

当 n 为奇数时，n 除以 4 的余数要么为1，要么为3

1. 如果为1，那么将 n 变为 (n - 1) / 2，这样得到的是偶数
2. 如果为3，那么将 n 变为 (n +1) / 2，这样得到的也是偶数（注意如果n为3，只需要两次即可，所以需要进行额外处理）
...
现在有两个长度相同的字符串X和Y，求出如果要将X和Y变成两个一样的字符串，需要的最少的代价之和是多少。

本题使用贪心的思想，为了使得修改的代价最小，需要让两个字符串的相对距离最小，因此可以先排序字符串，然后再遍历两个字符串，对应位置的字符相减
...
题目中如果提到连续，那么如果不满足条件就直接重置（如置为0），但是如果没提到连续，只是要求相对位置不改变，不满足条件时，可以考虑前一个状态或者后一个状态的最大值或最小值
...
[购物单_牛客题霸_牛客网](https://www.nowcoder.com/practice/f9c6f980eeec43ef85be20755ddbeaf4?tpId=37&tqId=21229&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3FtpId%3D37&difficulty=undefined&judgeStatus=undefined&tags=&title=)

背包问题的变体，有若干个物品可供购买，物品分为主物品和附属物品，一个主物品最多有两个附属物品，每个物品都有价格和重要程度，求固定预算下购买物品所得的最大满意度

1、满意度为价格和重要程度的乘积
2、购买附属物品前必须购买主物品


对于该问题，可以将其转换为背包问题，对于一个主物品，有四种可能，分别为只购买主物品、购买主物品+附属物品1、购买主物品+附属物品2以及购买主物品和所有的附属物品。
...
vs, ws, qs = [], [], []
for i in range(m):
    price, satisfaction, q = map(int, sys.stdin.readline().split())
    vs.append(price)
...
# 记录每种物品的可选情形
chooses = [[] for _ in range(m)]

for i in range(m):
    if qs[i] == -1:
        chooses[i].append((vs[i], ws[i] * vs[i]))  # 只选择主物品
        # 选择主物品和一个附属物品
        for it in items[i]:
            chooses[i].append((vs[i] + vs[it], ws[i] * vs[i] + ws[it] * vs[it]))
        # 如果有两个物品，再加上两个物品都选择的情况
        if len(items[i]) == 2:
            it1, it2 = items[i]
            chooses[i].append((vs[i] + vs[it1] + vs[it2], 
            vs[i] * ws[i] + vs[it1] * ws[it1] + vs[it2] * ws[it2]))
...
for i in range(1, m + 1):
    for j in range(n, -1, -10):
        max_choose = dp[j]
        for k in range(len(chooses[i - 1])):
            price, satisfaction = chooses[i - 1][k]
            if j >= price:
                max_choose = max(max_choose, satisfaction + dp[j - price])
        dp[j] = max_choose
print(dp[n])
```
...
令 `dp[i][j]` 为从 i 到 j 之间选择一个数字时确保获胜的最小现金数，可以在 i 到 j 之间选择一个数 k 分成两个区间，取其中较大的一个区间 
...
一般情况下可以使用双指针来判断某个序列是否是另一个序列的子序列
...
如果需要判断多个序列为某个序列的子序列，可以使用动态规划的方法，先预处理 t，令 `f[i][j]` 表示字符串 `t` 中从位置 `i` 开始往后字符 `j` 第一次出现的位置（包括自身）。
...
def isSubsequence(s: str, t: str) -> bool:
	...
	add = 0
	for i in range(n):
		if f[add][ord(s[i]) - ord('a')] == m:  # 如果后面没有对应的字符
			return False
		add = f[add][ord(s[i]) - ord('a')] + 1 # 跳转至第一次出现
...
本题实际在求最长整除子集（和求最长增长序列相似），问题的关键是如何根据动态规划的结果找到对应的子集

下面用一个例子说明如何得到最大整除子集。假设输入数组为 `[2,4,7,8,9,12,16,18]`，得到的动态规划表格如下：
...
2. 然后查找大小为 3 的最大整除子集，我们看到 8 和 12 对应的状态值都是 3，最大整除子集一定包含 8，这是因为 16 能够整除 8
...
def largestDivisibleSubset(self, nums: List[int]) -> List[int]:
	...
	last = nums[idx]   # 将last初始化为最后一个元素
	ans -= 1
	# 在前面的序列中找到长度减1并且能够整除的元素
	while ans >= 0 and idx >= 0:
		while idx >= 0:
			if dp[idx] == ans and last % nums[idx] == 0:
				break
			idx -= 1
		arr.append(nums[idx])
		last = nums[idx]   # 更新last
		ans -= 1
		idx -= 1
...
该题实际上是在求将数组分成两部分之后，使得两部分的差距最小，即求一半容量的背包装满时的重量

```python
def lastStoneWeightII(stones):
	tot = sum(stones)
	cap = tot // 2
	dp = [[0] * (cap+1) for _ in range(len(stones)+1)]
	...
	return tot - 2 * dp[-1][cap]
```

这里需要注意要有 i 这个维度，否则会出现塞入同一个石头的情况。

如果需要删除 i 这个维度，需要注意容量需要从后向前遍历，否则 `dp[j-w]` 实际对应的是 `dp[i+1][j-w]`，从后向前遍历可以保证因果性
...
给定两个正整数 `n` 和 `x`，求将`n` 表示成一些互不相同正整数的 `x` 次幂之和的方案数。

该问题实际上是背包问题，求背包的不同方案数
...
动态规划，需要注意初始化边界条件，`dp[i][j]` 表示 `s1[0:i]` 和 `s2[0:j]` 之间的编辑距离
    ...
    for i in range(1, n1+1):
        for j in range(1, n2+1):
            if s1[i-1] == s2[j-1]:
	            ...
	            # dp[i-1][j-1] 代表修改 s1 的第 i 个字符
	            # dp[i][j-1] 表示增加 s1 的第 i 个字符，
	            # dp[i-1][j] 表示删除 s1 的第 i 个字符
	...
	for i in range(1, n1+1):
		for j in range(1, n2+1):
			if s1[i-1] == s2[j-1]:
			    ...
			    # 选择删除 s1 中第 i-1 个字符，或者删除 s2 中第 j-1 个字符
...
为了保证不会出现剩余，对于无法切分的情况进行特殊标记，如设置为 -1，这样当分割到这种情况时可以避免计算
...
用栈保存下标，始终保持栈底元素为当前遍历过的元素的最后一个没有被匹配的右括号的下标，具体来说，保存所有 `(` 的下标，遇到 `)`，先弹出栈顶元素，表示匹配了当前右括号，如果栈为空，表明没有匹配，将其下标放入栈中，即没有被匹配的有括号的下标；如果栈不为空，当前有括号的下标减去栈顶元素即为以该右括号为结尾的最长有效括号的长度。
...
def solve_s(s):
    ...
    n = len(s)
    stack = []
    stack.append(-1)     # 处理最开始的括号作为平衡括号和第一个括号为右括号时的情况
...
还有另外一种思路，先从左向右遍历，记录左括号和右括号的数量，当左括号和右括号的数量相等时，证明存在一个合法的括号序列，可以更新结果，但是如果右括号的数量大于左括号，证明当前括号序列已经非法，将左右括号的数量置为0。

如果计数时，左括号的数量一直大于右括号的数量，那么就无法更新，可以反向再来一次
...
[873. 最长的斐波那契子序列的长度 - 力扣（LeetCode）](https://leetcode.cn/problems/length-of-longest-fibonacci-subsequence/description/)

在一个严格递增数组中找到最长的斐波那契数列

使用动态规划，令 `dp[i][j]` 表示以 `arr[i]` 和 `arr[j]` 结尾的斐波那契数列的长度，可以遍历数组确定最后一个数 k，然后用两个指针找到前面两个数 i 和 j，最后可以更新 `dp[j][k] = dp[i][j] + 1`
...
def solve(nums):
    ...
    for i in range(1, n):
        if nums[i] > stack[-1]:
            ...
            lb = bisect_left(stack, nums[i])  # 找到大于等于nums[i]的位置
            stack[lb] = nums[i]  # 替换这个元素使得增长变得更慢
    return len(stack)
```
...
首先需要对数组进行排序，排序的原则是宽度按照大小顺序排列，高度反向排列，排好之后。高度需要反向排是因为宽度已经排好，因此只需要比较高度，对于宽度相同的信封只能选择一个。由于高度反向排，因此首先选择的是最大高度的信封，这样其它信封就不会被选择了。如果高度也正向排，就会出现选择多个宽度相同的信封。


为了保证不超时，使用二分查找的方法根据第二个元素找到符合要求的子序列
...
def isMatch(s: str, p: str) -> bool:
	...
	for i in range(m+1):
		for j in range(1, n+1):
			if p[j-1] == "*":
				dp[i][j] |= dp[i][j-2]
				if match(i, j-1): # 如果s[:i] 和 p[:j-1]匹配,* 匹配字符
					dp[i][j] |= dp[i-1][j]
			else:
				if match(i, j):
					dp[i][j] |= dp[i-1][j-1]
...
变体：[字符串通配符_牛客题霸_牛客网](https://www.nowcoder.com/practice/43072d50a6eb44d2a6c816a283b02036?tpId=37&tags=&title=&difficulty=0&judgeStatus=0&rp=1&sourceUrl=%2Fexam%2Foj%2Fta%3Fpage%3D2%26tpId%3D37%26type%3D37)
...
def predictTheWinner(nums: List[int]) -> bool:
	...
	def dfs(l, r):
		...
		cnt1 = nums[l] + min(dfs(l+1, r - 1), dfs(l+2, r))

		cnt2 = nums[r] + min(dfs(l+1, r - 1), dfs(l, r - 2))
...
同时还有动态规划的解法，令 `dp[i][j]` 为数组剩下的部分为下标 i 到下标 j 时，即在下标范围 `[i,j]` 中，当前玩家与另一个玩家的分数之差的最大值。

如果 `i=j`，只剩一个数字，当前玩家只能拿取这个数字，因此对于所有 `0≤i<nums.length`，都有`dp[i][i]=nums[i]`。
...
该问题相当于求将数组顺序分成若干份，并求出每种分法中的最大值的最小值
...
def beautifulSubsets(nums, k: int) -> int:
	...
	for mask in range(1, 1 << n):
		arr = [nums[i] for i in range(n) if (mask >> i) & 1==1]
		flag = False
		for i in range(len(arr)):
...
该问题可以换一种思路思考，如果是戳气球，那么戳完气球后，顺序会发生变化，导致难以处理。因此，可以将戳气球改为放气球，最开始位置上没有气球，每次放一个气球，放上一个气球后，会将原本的区间分成左右两部分，如果一个部分还能继续放就对这个部分进行求解
...
def maxCoins(nums: List[int]):
	...
	def dfs(left, right):
		...
		for i in range(left+1, right):
			...
			total += dfs(left, i) + dfs(i, right)  # 求解左右区间
			best = max(best, total)
		memo[left][right] = best
		return best
	memo = [[-1 for i in range(n+2)] for i in range(n+2)]
	return dfs(0, n+1)
```

动态规划的方法解决，令 `dp[i][j]` 为在 `(i,j)` 开区间内放气球的最大收益
...
def maxCoins(nums: List[int]) -> int:
	...
	for i in range(n-1, -1, -1):   # 左端点从n-1到0
		for j in range(i+1, n+2):
			for k in range(i+1, j):  # 在 (i, j) 中放气球
				nr = vals[k] * vals[i]* vals[j]
				dp[i][j] = max(dp[i][j], nr + dp[i][k] + dp[k][j])
...
[2338. 统计理想数组的数目 - 力扣（LeetCode）](https://leetcode.cn/problems/count-the-number-of-ideal-arrays/?envType=daily-question&envId=2025-04-22)
...
这道题如果用一般的动态规划思想，令 `dp[i][j]` 为长度为 `i+1` 且结尾元素为 `j` 的理想数组数量，则有

```python
def idealArrays(n: int, maxValue: int):
	dp = [[0 for _ in range(maxValue+1)] for _ in range(n)]
	MOD = 10 ** 9 + 7
	for j in range(1, maxValue+1):
		dp[0][j] = 1   # 以 j 结尾的长度为 1 的平衡子数组只有一个
	
	for i in range(1, n):
		for j in range(1, maxValue+1):
			for k in range(j, maxValue+1, j):  
			# i + 1 长度且结尾为 k 的理想数组是长度为 i 且结尾为j的理想数组，k为j的倍数
				dp[i][k] = (dp[i][k] + dp[i-1][j]) % MOD
	cnt = 0
	for j in range(1, maxValue+1):
		cnt = (cnt + dp[n-1][j]) % MOD
	return cnt
```

但是这样的复杂度较高，容易超时

由于本题允许重复，并且理想数组是非严格递增，因此只需要考虑不同元素的个数，如果 maxValue 小于$10^4$，那么最多有$\log_2(10^4) = 13$ 个因子 （即 1、2、4、8、...、$2^{13}$）

令 $k_i = \frac{a_i}{a_{i-1}}$，则一个理想数组对应着一个商分数组，如 $a=[1, 4, 4, 8, 8]$ 的商分数组 $k=[1,4,1,2,1]$。根据 k 的定义可以得到 k 中所有元素的乘积等于 a 中的最后一个元素。所以可以将问题转换为已知理想数组的最后一个元素，求商分数组的数量。

可以先对最后一个元素进行质因子分解，统计每种质因子的数量，可以使用枚举法，从 i=2 开始枚举，如果 x 能被 i 整除，就不断地除 i，直到 x 不能被 i 整除为止，统计除 i 的次数，即为 x 中的质因子 i 的出现次数。当计算好一个质因子的数量 $c$ 时，我们需要考虑存在多少种放置方法，假设不同因子的数量为 $n$，则相当于将 $c$ 个相同的因子放在 $n$ 个不同的位置中，相当于有 n + c 个位置，将其划分为 n 份，即 $C(n+c-1, n-1) = C(n+c-1, c)$
...
# EXP[x] 为x分解后每个质因数的出现次数（只需要关注次数，而不需要关注质因子到底是什么）
...
# 预处理组合数 从 i 中选 j 个元素，对于第 i 个元素来说分成两种情况
# 选（C[i - 1][j - 1]）和不选（C[i-1][j]）
...
[218. 天际线问题 - 力扣（LeetCode）](https://leetcode.cn/problems/the-skyline-problem/description/)

采用类似归并算法思路，两两分组最终得到一个元素，该元素对应的两个天际线，分别为 (起点，高度) 和 (终点，0)，然后再合并。合并时用两个变量保存两部分当前的高度，每次保存最大值。这样得到结果会存在重复，最后进行删除
...
def solve(arr):
    ...
    def merge(sky1, sky2):
        i,j, h1, h2 = 0, 0, 0, 0   # h1保存sky1当前高度，h2保存sky2当前高度
        ...
        while i < n1 and j < n2:
            if sky1[i][0] < sky2[j][0]:
                ...
                res.append((l1, maxh))
                i += 1
            elif sky1[i][0] == sky2[j][0]:   # 处理两个端点相等时的情况
				h1, h2 = sky1[i][1], sky2[j][1]
				h = max(sky1[i][1], sky2[j][1])
				res.append([sky1[i][0], h])
				i += 1
				j += 1
            else:
                l2, h2 = sky2[j]
...
对于一个字符串如果存在一个字符 ch，其出现次数大于0且小于k，则任何包含该字符的子串都不可能满足要求。也就是说，我们将字符串按照 、 切分成若干段，则满足要求的最长子串一定出现在某个被切分的段内，而不能跨越一个或多个段。因此，可以考虑分治的方式求解本题。

```python
def longestSubstring(s: str, k: int) -> int:
	def dfs(s, l, r, k):
		cnt = dict(Counter(s[l:r+1]))
		split = None
		# 找到分割字符 split
...
[241. 为运算表达式设计优先级 - 力扣（LeetCode）](https://leetcode.cn/problems/different-ways-to-add-parentheses/description/)

该问题可以通过分治的方法解决。首先先解析字符串，将数字和操作符分别放在一个数组中。
...
可以使用 DFS 或者 BFS 解决，以 BFS 为例，用一个队列保存当前状态，将空白区域向四个方向移动，得到四个状态（需要用一个矩阵保存已经访问过的状态）
