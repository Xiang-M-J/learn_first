...
1. `git status`： 查看当前git状态，用于判断是否已经准备好推送分支
    
2. `git add .`: 添加所有的文件
    
3. `git commit -m "..."`: 添加推送时的描述
    
4. `git remote add origin git网址`: 连接要推送的目的网址
    
5. `git push -u origin 分支名称`: 推送代码
    
6. `git checkout -b 新分支名称`: 切换到一个新的分支
    
7. `git branch`: 查看当前所有分支
    
8. `git log`: 查看git的提交版本和id，可用于查看需要恢复的版本号
    
9. `git reset --hard 版本号`: 恢复到指定版本
    
10. `git push -f origin 分支名称`: 强制push
    
11. `git clone git网址`: 克隆一个项目到本地
    
12. `git branch 目标分支名`: 切换至目标分支
    
13. `git merge 目标分支名`: 将目标分支与本分支合并
    
14. `git push`: 分支合并后推送至云端
    

git分为本地仓库和远程仓库，一般情况写完代码，commit到本地仓库（生成本地仓的commit ID，代表当前提交代码的版本号），然后push到远程仓库（记录这个版本号）。
...
:warning: **`nginx`的配置文件名字只能是`nginx.conf`**

:warning: `nginx -t -c conf/nginx.conf`：在测试配置文件时**不要**在conf文件夹进行测试，否则即使配置文件编写正确也会报错
:warning: 当用脚本执行`start nginx`，应该先进入nginx所在文件夹
...
```shell
cmake .   # 配置项目并生成本机构建系统（Windows下生成.sin文件）
cmake --build .		# 编译、连接项目
cd Debug			# 进入Debug
Tutorial			# 执行文件
```
