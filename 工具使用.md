...
下载地址：[Zotero | Your personal research assistant](https://www.zotero.org/downloads)

插件商店：[Zotero 插件商店 | Zotero 中文社区](https://zotero-chinese.com/plugins/)

推荐插件：

1. Actions and Tags for Zotero：[Customize your Zotero workflow.](https://github.com/windingwind/zotero-actions-tags)
2. Better Notes for Zotero：[Better Notes | Zotero 中文社区](https://zotero-chinese.com/user-guide/plugins/better-notes)
3. Del Item With Attachment：[Remove attachment(s) when delete the item(s) or collection in Zotero and JurisM.](https://github.com/redleafnew/delitemwithatt)
4. Ethereal Style：[Ethereal Style for Zotero | Zotero 中文社区](https://zotero-chinese.com/user-guide/plugins/style)
5. Google Scholar Citation Count：[Zotero plugin for fetching number of citations from Google Scholar.](https://github.com/justinribeiro/zotero-google-scholar-citation-count)
6. Jasminum：[茉莉花 | Zotero 中文社区](https://zotero-chinese.com/user-guide/plugins/jasminum)
7. Translate for Zotero：[Translate for Zotero | Zotero 中文社区](https://zotero-chinese.com/user-guide/plugins/translate/)


在word中插入引用：[在 Word 中插入参考文献 | Zotero 中文社区](https://zotero-chinese.com/user-guide/ms-word-plugin)

引文格式可以使用[GB-T-7714—2015（顺序编码，双语，姓名不大写，无URL、DOI，引注有页码）](https://zotero-chinese.com/styles/GB-T-7714%E2%80%942015%EF%BC%88%E9%A1%BA%E5%BA%8F%E7%BC%96%E7%A0%81%EF%BC%8C%E5%8F%8C%E8%AF%AD%EF%BC%8C%E5%A7%93%E5%90%8D%E4%B8%8D%E5%A4%A7%E5%86%99%EF%BC%8C%E6%97%A0URL%E3%80%81DOI%EF%BC%8C%E5%BC%95%E6%B3%A8%E6%9C%89%E9%A1%B5%E7%A0%81%EF%BC%89/)
如果出现英文文献引用时出现等号，可以使用Del Item With Attachment插件将文献的语言改为en-US。
...
用于构建内容驱动网站的web框架，支持主流的UI框架（react、vue等），可以用来写博客、文档等。
...
![](images/tools/astro_tmpl.png)

Astro项目会有以下项目

+ `src/*`：项目源代码
+ `public/*`：一些资源文件（字体、图标等）
+ `package.json`：项目依赖文件
+ `astro.config.mjs`：Astro配置文件
+ `tsconfig.json`：typescript配置文件

`src/` 目录是项目的大部分源码所在位置。包括但不限于：
...
- [Images to be optimized and processed by Astro](https://docs.astro.build/en/guides/images/)
 ...
 构建处理：Astro 对 `src/` 中的文件会进行处理、优化、打包。不是所有文件都原封不动地到浏览器，比如 Astro 组件会渲染成静态 HTML。虽然文档推荐某些目录结构 (`components/`、`layouts/`、`styles/`)，但你有较大的灵活性：除 `src/pages/` 之外，其它目录可以自由命名和组织。

`src/pages/` 目录

- 必须存在：`src/pages/` 是 Astro 项目中非常核心的目录。文档明确指出，如果缺少这个目录，网站将没有页面/路由。
- 该目录下的文件会被 Astro 用来生成页面 (基于文件路由) ——你在这里放 `.astro`、`.md`、`.mdx` 等页面内容。

`src/components/` 目录

- 用于存放可重用组件 (Astro 组件 + UI 框架组件) 。这是一个社区约定的结构，但不是强制要求。 
- 这样做有助于将 UI 的不同部分模块化和复用：按钮、导航条、卡片等等组件可以统筹管理。

`src/layouts/` 目录

- 布局 (Layout) 是一种 Astro 组件，用来定义多个页面共享的 UI 结构 (例如头部、导航、页脚等) 。
- 布局组件里通常包括一个 `<slot />`，页面内容会被注入到 slot 里。 (这是布局设计的一般方式) 
- 虽然这是推荐做法 (约定俗成)，但不是强制的。你也可以把布局组件放在别处，甚至和页面放在一起。

`src/styles/` 目录

- 这是一个常见用来存储 CSS / Sass 文件的目录。将样式文件放在 `src/styles/` 是推荐做法，但不是必须的。
- 重要的是，样式要在 `src/` 目录里 (或其它地方被正确 import)，Astro 才会处理和优化它们。 

`public/` 目录

- `public/` 用来存放 **无需构建处理 (build) 的静态资产**：例如字体 (fonts)、图标 (icons)、`robots.txt`、`manifest.webmanifest` 等。 
- 这些文件会在构建过程中被 **原封不动地复制** 到最终输出目录。
- 虽然你可以把 JS 或 CSS 文件放在 `public/`，但这样做的话它们 **不会被 Astro 打包或优化**。因此，一般建议你自己写的 CSS / JS 放在 `src/`。 

`package.json`

- 这个文件是项目的 manifest，用来管理依赖 (dependencies) 和开发依赖 (devDependencies)，定义脚本 (如 `dev`、`build`)。 
  
- Astro 推荐把大部分依赖 (尤其是构建时/运行时会用到的) 放在 `dependencies`。只有在确实是开发阶段才考虑 `devDependencies`。 

 `astro.config.mjs` (或 `.js` / `.ts`)

- 这个是 Astro 的配置文件。你可以在这里定义：集成（integrations）、构建选项、服务器选项等。 
- Astro 支持多种配置文件格式： `.mjs`, `.js`, `.cjs`, `.ts` 等。根据你是否使用 TypeScript 可选择不同格式。
- 如果你用 TypeScript 的 `.ts` 配置文件，Astro 会使用 `tsm` 来加载它，并遵循你的 `tsconfig.json`。

`tsconfig.json`

- 这是 TypeScript 的配置文件 (如果你在项目中使用 TypeScript 或希望编辑器更好支持 import 等功能的话非常推荐) 。 
- 虽然不是严格“必须” (如果你不写 TS 也可以不用)，但没有它的话，有些编辑器功能 (比如某些包导入提示) 会受限。
...
1. 首先创建一个astro项目，可以选择blog模板，选择创建git；
2. 在GitHub中创建一个仓库`astro_site`用来同步本地的astro项目；
3. 在本地项目中的`.github/workflows/deploy.yml`，内容如下
```yml
...
2. 配置`astro.config.mjs`，添加site和base
```
...
3. 在远程仓库的设置页面，找到Pages页面，选择Source为Github Actions

![](images\tools\github_pages.png)
在选择完Github Actions后，可以选择browse all workflows，选择astro，这样会在`.github/workflows`创建`astro.yml`。
...
一款静态代码分析和规范化工具，适用于python，比Flake8（代码分析工具）和Black（规范化工具）快很多。

Ruff可以作为python库下载，然后直接在需要分析或者规范化的文件夹下执行命令即可

```sh
ruff check
```

使用`ruff format`命令规范化文件
...
安装完uv后，需要添加一个环境变量`UV_DEFAULT_INDEX`，这个变量的值和pip配置镜像源时的地址一致，如https://pypi.tuna.tsinghua.edu.cn/simple。

以下是根据 uv（“uv”）官方文档“Features（特性）”一节整理的中文笔记，方便你快速理解其功能模块与使用场景。

## 一个例子

创建项目并且指定python版本，不需要指定小版本，指定3.x即可
...
> 一定要加uv run，否则会使用系统默认的python执行程序。
...
功能：安装、列出、查找、绑定、卸载不同的 Python 版本，为项目提供多版本支持。 
常用命令示例：

- `uv python install`：安装一个 Python 版本。
- `uv python list`：列出可用／已安装的 Python 版本。
- `uv python find`：查找已安装的某个 Python 版本。
- `uv python pin`：将当前项目绑定（pin）使用特定 Python 版本。
- `uv python uninstall`：卸载某个 Python 版本。 
  

适用场景：
- 当你需要在一个开发环境管理多个 Python 版本时。
- 项目明确要求使用特定 Python 版本，以确保一致性。
- 想简化版本切换、绑定项目与版本关系。

详见 [Installing and managing Python | uv](https://docs.astral.sh/uv/guides/install-python/)

### 脚本

功能：用于运行单个 Python 脚本，并为脚本添加／移除依赖。适合轻量级脚本型使用场景。
典型命令：

- `uv run`：执行一个脚本（如 `example.py`） 。
- `uv add --script`：向脚本添加一个依赖包。
- `uv remove --script`：从脚本依赖中移除一个包。

适用场景：
- 快速运行小测试脚本、工具脚本，而无需建立完整项目结构。
- 在脚本级别轻松管理依赖。

详见[Running scripts | uv](https://docs.astral.sh/uv/guides/scripts/#running-a-script-without-dependencies)

### 项目

功能：用于管理 Python 项目的生命周期 — 包括初始化、依赖管理、锁定、运行、构建、发布。
主要命令包括：

- `uv init`：初始化一个新的 Python 项目（生成 `pyproject.toml` 等）。
- `uv add`：向项目中添加依赖。
- `uv remove`：从项目中移除依赖。
- `uv sync`：同步项目依赖到环境（即安装／调整包以匹配定义）。
- `uv lock`：生成锁定文件（lockfile），确保依赖版本可复现。
- `uv run`：在项目的环境中运行命令。
- `uv tree`：查看项目的依赖树。
- `uv build`：将项目打包成分发档案（比如 wheel 或 sdist）。 
- `uv publish`：将项目发布到包索引。

适用场景：
- 开发中大型 Python 项目，需要版本控制、依赖锁定、构建、发布。
- 团队协作开发、CI/CD流程中，保证环境一致性。
- 从脚本发展成模块／包时，推荐转向此模块。

详见[Working on projects | uv](https://docs.astral.sh/uv/guides/projects/)


### 工具

功能：管理和运行那些发布在 PyPI 等包索引、但一般作为工具（而非库）使用的软件。例如格式化器（如 `black`）、静态分析器（如 `ruff`）等。
命令示例：
- `uvx` / `uv tool run`：在一个临时环境中运行工具。
- `uv tool install`：安装一个工具（用户级别）。
- `uv tool uninstall`：卸载一个工具。 
- `uv tool list`：列出已安装的工具。
- `uv tool update-shell`：更新 shell，使得工具可执行程序被正确发现。

适用场景：
- 你希望安装或管理一些命令行工具（如 linters、formatters）而不是项目依赖。
- 希望工具与项目隔离，或能在不同项目间共享。

详见[Using tools | uv](https://docs.astral.sh/uv/guides/tools/)

### pip 接口

功能：为那些更习惯传统 `pip`／`venv` 工作流程的用户提供支持。即便主推 uv 的高层命令，但仍支持环境／包管理的“底层”流程。 
相关操作包括：

- 创建虚拟环境：`uv venv`，详见[Using environments | uv](https://docs.astral.sh/uv/pip/environments/#creating-a-virtual-environment)
- 包管理：`uv pip install`、`uv pip show`、`uv pip freeze`、`uv pip check`、`uv pip list`、`uv pip uninstall`、`uv pip tree`，详见 [Managing packages | uv](https://docs.astral.sh/uv/pip/packages/)
- 锁定环境：`uv pip compile`（生成 lockfile）、`uv pip sync`（同步环境与 lockfile），详见[Locking environments | uv](https://docs.astral.sh/uv/pip/compile/)
  
>使用时可能与原生 `pip`、`pip-tools` 的行为有差异，因此如果你严格依赖某些 `pip` 行为，要先查阅[兼容性文档](https://docs.astral.sh/uv/pip/compatibility/)。


### 实用功能

功能：管理 uv 本身的状态，如缓存、安装目录、Python 版本目录、更新 uv 本身。
典型命令：
- `uv cache clean`：清除缓存条目。
- `uv cache prune`：删除过期缓存条目。
- `uv cache dir`：显示 uv 的缓存目录路径。
- `uv tool dir`：显示工具安装目录。
- `uv python dir`：显示 uv 安装的 Python 版本目录路径。
- `uv self update`：将 uv 更新至最新版本。 

适用场景：

- 管理 uv 的安装环境、清理旧版本、查看目录路径。
- 当 uv 自己的版本过旧或行为异常时进行更新
- 优化系统磁盘使用，清理缓存。
...
先关闭word等office软件，安装完EndNote后，word中会有对应的插件

使用方法如下

1、在google scholar 中搜索文献，并将文件保存到我的图书馆中

![](images\endnote\google_scholar_save.png)

2、将所有的文献全部保存好之后，在我的图书馆中点击全部导出->endnote

![](images\endnote\google_scholar_export_all.png)

3、导出后的文件双击即可导入endnote中，如果需要插入word，可以直接将光标放到插入的位置上，然后在Endnote中点击插入

![](images\endnote\endnote_insert.png)
...
只要是菜单栏上有的命令，可以将软件语言设置为英文后查看对应的文字来确定快捷键调用的命令。
...
> 可以直接在Github上下载编译好的库
> 然后用 cmake .. -G Ninja -DCMAKE_BUILD_TYPE=Release -DMNN_BUILD_SHARED_LIBS=OFF -DMNN_WIN_RUNTIME_MT=ON -DMNN_BUILD_CONVERTER=ON 编译转换器



编译方法参考 [主库编译 — MNN-Doc 2.1.1 documentation](https://mnn-docs.readthedocs.io/en/latest/compile/engine.html#windows-visual-studio-clang) 中的clang编译

首先需要参考[Visual Studio 项目中的 Clang/LLVM 支持 | Microsoft Learn](https://learn.microsoft.com/zh-cn/cpp/build/clang-support-msbuild?view=msvc-170) 安装clang工具链，可能还需要安装 Win10 的SDK。

编译指令为（下面编译的是Release版本的，还可以换成Debug版本）

```
cmake .. -G Ninja -DCMAKE_C_COMPILER="D:\VisualStudio\Community\VC\Tools\Llvm\x64\bin\clang.exe" -DCMAKE_CXX_COMPILER="D:\VisualStudio\Community\VC\Tools\Llvm\x64\bin\clang++.exe" -DCMAKE_LINKER="D:\VisualStudio\Community\VC\Tools\Llvm\x64\bin\lld.exe" -DCMAKE_BUILD_TYPE=Release
```
...
这里可能会报错 `lld-link: error: undefined symbol: strdup`

这是因为Windows中没有strdup函数，可以在有这个函数的文件中加上宏定义（需要保证引入了string.h）
...
编译成功后会在build文件夹中生成 MNN.lib 和 MNN.dll
...
```sh
MNNConvert.exe -f ONNX --modelFile msa_dpcrn_fast.onnx --MNNModel msa_dpcrn.mnn
```

对于pb模型

```sh
MNNConvert.exe -f TF --modelFile model-mobilenet_v1_075.pb --MNNModel mobile.mnn
```

### 在 Visual Studio 中使用

先创建一个项目，设置项目属性->C/C++ -> General -> Additional Include Directories 中包括 `E:\work\inference_arch\MNN\include`

设置项目属性 -> Linker -> General -> Additional Library Directories 中包括 `E:\work\inference_arch\MNN\build`

设置项目属性 -> Linker -> Input -> Additional Dependencies 中包括 MNN.lib

在运行项目时，会提示找不到 MNN.dll，可以手动将 dll 文件复制到程序所在文件夹。
...
选择在其他盘中安装 Linux 系统，参考[Win10自定义路径位置安装WSL2 (Ubuntu 20.04) 并配置CUDA_wsl安装路径-CSDN博客](https://blog.csdn.net/weixin_41973774/article/details/117223425)
...
4、 创建并切换到你希望推送的新分支（比如 `new-branch`）
...
![image-20231228194632204](images\工具使用\image-20231228194632204.png)
