...
WeNet中的默认分词方法为直接拆分文本（英文按照空格，中文按照汉字），将拆分后的文本按照字典序排序，然后去重，最后为每个字符分配一个索引。默认会为 `<blank>`、`<unk>`和`<sos/eos>`这三个特殊字符分配0，1，2这三个索引。

WeNet在加载音频数据时分为两种情况，一种是直接加载音频，另一种是shard格式（详见 tools/make_shard_list.py）先将音频（加载为int16格式）加载到一个二进制文件，在一个文本中记录该二进制文件的名字，方便读取大量音频。

>读取大量数据的另一种方法是使用webdataset、Nvidia Dali，Meta SPDL、Ray Data等库。

训练完成后，WeNet会融合几个权重（直接平均），然后再进行推理。

>wenet中的一些示例中的yaml文件中缺少某些属性，如缺少 input_dim

# FairSeq

一个用于序列到序列建模的工具库

先拉取fairseq代码，再以可编辑形式安装便于后续开发
...
> fairseq似乎不支持新的python，python3.9支持

fairseq训练模型时容易报错：FileExistsError: WinError 183 当文件已存在时，无法创建该文件。这种情况下可以修改`fairseq\file_io.py`，将其中的
...
Github地址：[fairseq_tasks/speech_separation at main · Xiang-M-J/fairseq_tasks](https://github.com/Xiang-M-J/fairseq_tasks/tree/main/speech_separation)

先在fairseq文件夹中创建一个文件夹如research用于存放所有自定义的项目，在research中创建speech_separation任务文件夹存放语音分离模型的定义。需要编写如下内容
...
criterions定义损失函数等，损失函数需要继承FairseqCriterion类，同时使用`register_criterion`注册。fairseq中的损失需要返回一个batch的**损失之和**，而非均值。FairseqCriterion类中的`reduce_metrics`函数可用于记录损失，注意需要除以样本数。
...
模型定义需要继承BaseFairseqModel，使用`register_model`注册，模型初始化时使用args传入所有参数。为了方便调用模型，还会定义一些默认参数配置的模型

```python
@register_model_architecture("conv_tasnet_fairseq", "conv_tasnet_base")
def base_architecture(args):
    args.enc_dim = getattr(args, "N", 512)  # N: 编码器维度
    args.kernel_len = getattr(args, "L", 16)  # L: 卷积核长度
    args.sep_dim = getattr(args, "B", 128)  # B: 瓶颈层维度
    args.hidden_dim = getattr(args, "H", 512)  # H: 卷积块通道
    args.sep_kernel = getattr(args, "P", 3)  # P: TCN 卷积核大小
    args.num_blocks = getattr(args, "X", 8)  # X: 每个 Repeat 里的 block 数
    args.num_repeats = getattr(args, "R", 3)  # R: 重复次数
    args.num_sources = getattr(args, "num_spks", 2)  # 分离源数量

```

tasks文件夹中可以定义数据集和任务类，同样需要继承对应的类。训练类中可以添加任务特定的命令行参数，训练的命令如下所示，第一个参数为数据存放位置，`--user-dir`指定任务文件夹位置，`--task` 指定任务类，`--arch`指定模型架构，`--criterion`指定损失函数。

```bash
fairseq-train ../data/wsj0-2mix \
    --user-dir ./speech_separation \
    --task speech_separation_task \
    --arch conv_tasnet_base \
    --criterion si_snr_pit \
    --n-srcs 2 \
    --batch-size 8 \
    --num-workers 0 \
    --optimizer adam --lr 0.0005 \
    --save-dir ./speech_separation/checkpoints/ \
    --max-epoch 50
```
...
对于某些任务需要安装额外的依赖库，如k2、flair等。

k2安装时参考[From pre-compiled wheels (recommended) — k2 1.24.4 documentation](https://k2-fsa.github.io/k2/installation/from_wheels.html)，使用编译好的wheel

flair需要transformers库，transformers库的版本不能过高，可选的版本为4.36.2。

运行speechbrain中的示例时，如果数据预处理出错，需要注意删除结果文件夹，否则后续可能会跳过这些步骤导致报错。
...
>可以使用[OnnxSlim](https://github.com/inisis/OnnxSlim)、[onnxsim](https://github.com/onnxsim/onnxsim)等工具优化onnx格式模型
