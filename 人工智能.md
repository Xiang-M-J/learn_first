...
参考GitHub项目：[karpathy/micrograd: A tiny scalar-valued autograd engine and a neural net library on top of it with PyTorch-like API](https://github.com/karpathy/micrograd)

首先需要创建一个类，用来表示张量（为了简单，只考虑单个数字的张量），这个类具备自动求导的能力。实现加法、乘法、幂次和relu函数的前向和反向，其它如减法、除法在此基础上实现。为了反向传播梯度，构建一个图，输出作为父节点，输入作为子节点。
...
可以编写一个demo验证正确性，标签为1或者-1，损失函数为hinge loss
...
在混合精度训练时，权重、激活和梯度保存为FP16格式，为了匹配32位精度的网络，会保存一份FP32格式的权重备份。每次迭代时，在FP16格式下进行前向计算和梯度反向传播，然后将FP16精度的梯度应用到备份的权重上。

由于FP16精度的表示范围小，当值低于可表示范围时，数值会变为0。为了缓解这一问题，会将损失乘上缩放因子（缩放因子的值可以在8至32K范围内选择，是一个经验值），计算完梯度后再除以缩放因子。

混合精度训练对于模型的精度影响很小，甚至一些情况下会提升模型性能。

> 现代混合精度训练时，常使用BF16精度，该精度的表示范围和FP32相近，一般无需对损失进行缩放，但是仅在较新的显卡设备上得到支持
...
1. 自回归模型，使用观测序列 $x_{t−1},\cdots,x_{t−\tau}$ 来估计 $x_t$。
2. 隐变量自回归模型，保留一些对过去观测的总结 $h_t$，并且同时更新预测 $\hat x_t$ 和总结 $h_t$。其中，$\hat x_t = P(x_t|h_t)$，$h_t=g(h_{t-1},x_{t-1})$。
...
对于 RNN 训练语言模型时，可以将输入序列向后移动一位作为输出序列。对于训练好的 RNN 模型，输入文本，预测后面文本的代码如下：
```python
...
已知观测到一组由未知主体触发的数据，每组数据由单个主体触发。假设这些事件由两个主体触发，现在需要计算两个主体发生某个事件的概率，可以使用em算法

1. 先初始化两个主体发生该事件的概率，如假定均为0.5；
2. 计算在两个主体发生该事件的概率下，每组数据由两个主体触发的概率；
3. 计算得到每组数据由两个主体触发的概率后，更新两个主体发生该事件的概率

下面是一个例子，假设有两个硬币，两个硬币每个投10次，记录一组数据（数据和硬币的对应关系未知），求两个硬币投出正面的概率

```python
import numpy as np

# 一共5组数据，每组数据由单个未知主体产生，
# 单个主体产生的数据以[a, b]形式表示，a为a事件发生的次数，b为其它事件发生的次数
...
for i in range(10):
    # probs为每组数据由两个主体产生的概率
    probs = [[0, 0] for _ in range(len(data))]
    for j in range(len(data)):
        # 第1个主体触发第j组数据的概率
        prob1 = p1 ** data[j][0] * (1 - p1) ** data[j][1]
        # 第2个主体触发第j组数据的概率
        prob2 = p2 ** data[j][0] * (1 - p2) ** data[j][1]
        probs[j][0] = prob1 / (prob1 + prob2) # 第j组数据由第1个主体触发的概率
        probs[j][1] = prob2 / (prob1 + prob2)

    probs = np.array(probs)
    # 根据每组数据由两个主体触发的概率，计算每个主体发生a事件的概率
    p1 = sum(probs[:, 0] * data[:, 0]) / sum(probs[:, 0] * np.sum(data, axis=-1))
    p2 = sum(probs[:, 1] * data[:, 0]) / sum(probs[:, 1] * np.sum(data, axis=-1))

print(p1)
print(p2)
```

第二个例子：假设有50个男生身高和50个女生身高混在一起，男生和女生的身高分布为高斯分布，现在需要求出男生身高和女生身高的分布参数。
...
# 生成数据
heights = np.concatenate((np.random.normal(m_mean, m_std, 1000), np.random.normal(f_mean, f_std, 1000)))
random.shuffle(heights)

# 估计值
pm_mean, pm_std = 175, 10
pf_mean, pf_std = 160, 8

# 开始迭代
for i in range(1000):
    # 记录哪些是男性，哪些是女性
    prob_m = np.abs(heights - pm_mean) / pm_std
    prob_f = np.abs(heights - pf_mean) / pf_std
    males = prob_m > prob_f
    females = prob_f > prob_m
    
    pm_mean, pm_std = np.mean(heights[males]), np.std(heights[males])
    pf_mean, pf_std = np.mean(heights[females]), np.std(heights[females])

print(f"male mean: {pm_mean}, std: {pm_std}")
print(f"female mean: {pf_mean}, std: {pf_std}")
```

如果只有50个男生和女生，估计不太准确，但是有1000个男生和女生时，估计就比较准确了
...
在训练模型时通过ema来稳定验证时的模型参数，具体做法是维护一个影子权重，初始时，影子权重 $v_t$ 直接deepcopy原模型权重 $\theta_t$。每训练一定步数，利用ema更新影子权重：

$$
v_t = \beta \cdot v_{t-1} + (1-\beta) \cdot \theta_t
$$
这里的$\beta$一般会取一个比较大的值如0.9-0.999。在验证和保存模型时会使用影子权重 $v_t$，然后恢复为模型权重$\theta_t$进行后面的训练。

下面解释ema为什么有效，令$n$时刻的模型权重为 $\theta_n$，梯度为 $g_n$，影子权重为$v_n$，则有
$$
{\theta _n} = {\theta _{n - 1}} - {g_{n - 1}} = {\theta _{n - 2}} - {g_{n - 1}} - {g_{n - 2}} =  \cdots  = {\theta _1} - \sum\limits_{i = 1}^{n - 1} {{g_i}} 
$$
$$
{v_n} = \beta  \cdot {v_{n - 1}} + (1 - \beta ) \cdot {\theta _n} =  \cdots  = {\beta ^n} \cdot {v_0} + (1 - \beta ) \cdot \left( {{\theta _n} + \beta {\theta _{n - 1}} +  \cdots  + {\beta ^{n - 1}}{\theta _1}} \right)
$$
将 $\theta_n$代入$v_n$中可以得到
$$
{v_n} = {\beta ^n} \cdot {v_0} + (1 - {\beta ^n}){\theta _1} - \sum\limits_{i = 1}^{n - 1} {\left( {1 - {\beta ^{n - i}}} \right)} {g_i} = {\theta _1} - \sum\limits_{i = 1}^{n - 1} {\left( {1 - {\beta ^{n - i}}} \right)} {g_i}
$$
可以看到验证时使用的权重实际上为模型权重加上学习率衰减的梯度，使得验证过程更加稳定。

除了根据batch进行平均还有根据epoch进行平均，可以参考[starrytong/SCNet](https://github.com/starrytong/SCNet)中的实现，默认情况下设置了四种ema方案，两种根据batch，两种根据epoch

[Switch EMA](https://arxiv.org/abs/2402.09240)提出在每个epoch后使用影子权重更新模型权重
...
扩散模型可以从ODE的角度解释，假设数据分布符合$x\sim p_{data}(x)$，噪声分布符合$\varepsilon \sim p_{noise}(\varepsilon)$，训练时，通过插值来采样噪声样本，$z_t = a_t x+b_t\epsilon$，$a_t$和$b_t$是采样策略，如$a_t=t, b_t = 1-t$，$t$符合logitnormal分布。当t为0时$z_t$为噪声，t为1时为数据。flow velocity $v$定义为$z_t$的导数，即$v_t = z'_t=a'_t x - b'_t \varepsilon$
$$
v = x-\varepsilon
$$
基于流的方法通过最小化模型预测的$v_{\theta}(z_t,t)$和$v$之间的mse损失。

**预测空间**：网络直接的输出可以为 $v$、$x$或者 $\varepsilon$，给定$(v, x, \varepsilon)$这三个未知量和一个网络的输出，再加上上面的插值方程和 $v$ 的计算公式，可以求解这三个未知量。假设网络输出$net_{\theta}$为$x$，则可以得到下面这个方程组

$$
\left\{ \matrix{
  {x_\theta } = ne{t_\theta } \hfill \cr 
  {z_t} = t{x_\theta } + (1 - t){\varepsilon _\theta } \hfill \cr 
  {v_\theta } = {x_\theta } - {\varepsilon _\theta } \hfill \cr}  \right.
$$
求解这个方程组可以得到$\varepsilon_{\theta}$和$v_{\theta}$。

**损失空间**：$v$、$x$或者 $\varepsilon$都可以作为损失，如$v$损失定义为

$$
\eqalign{
  & L = E{\left\| {{v_\theta } - v} \right\|^2} = E{\left\| {\left( {{x_\theta } - {\varepsilon _\theta }} \right) - \left( {x - \varepsilon } \right)} \right\|^2}  \cr 
  &  = E{\left\| {\left( {{x_\theta } - {{{z_t} - t{x_\theta }} \over {1 - t}}} \right) - \left( {x - {{{z_t} - tx} \over {1 - t}}} \right)} \right\|^2}  \cr 
  &  = E{\left\| {{{{x_\theta }} \over {1 - t}} - {x \over {1 - t}}} \right\|^2} = E{1 \over {{{\left( {1 - t} \right)}^2}}}{\left\| {{x_\theta }({z_t},t) - x} \right\|^2} \cr} 
$$

> JiT采用了预测$x$，$v$损失的方式训练模型。
...
音频编码器，支持44.1kHz，24kHz和16kHz，模型权重可在releases中下载

RVQGAN使用Residual Vector Quantization来量化编码，感觉量化后更大了，如果想要减小维度，还是用soloaudio中的量化方法比较好。
...
原始代码：[salesforce/pytorch-qrnn: PyTorch implementation of the Quasi-Recurrent Neural Network - up to 16 times faster than NVIDIA's cuDNN LSTM](https://github.com/salesforce/pytorch-qrnn)

原始代码版本较低，修改后的代码为：[Xiang-M-J/pytorch-qrnn: Rewrote ForgetMult using CuPy.](https://github.com/Xiang-M-J/pytorch-qrnn)

使用时最好堆叠多层

### SRU

论文：[Simple Recurrent Units for Highly Parallelizable Recurrence](https://arxiv.org/abs/1709.02755)

代码：[asappresearch/sru: Training RNNs as Fast as CNNs (https://arxiv.org/abs/1709.02755)](https://github.com/asappresearch/sru)
...
论文：[Parallelizing Linear Recurrent Neural Nets Over Sequence Length](https://arxiv.org/abs/1709.04057)

代码：[eamartin/parallelizing_linear_rnns](https://github.com/eamartin/parallelizing_linear_rnns)


### mingru

论文：[Were RNNs All We Needed?](https://arxiv.org/abs/2410.01201)

代码：[lucidrains/minGRU-pytorch: Implementation of the proposed minGRU in Pytorch](https://github.com/lucidrains/minGRU-pytorch)

和girl挺像的

### xlstm

论文：[xLSTM: Extended Long Short-Term Memory](https://arxiv.org/abs/2405.04517)

代码：[NX-AI/xlstm: Official repository of the xLSTM.](https://github.com/NX-AI/xlstm)
...
1. [PANNs](https://github.com/qiuqiangkong/audioset_tagging_cnn)
2. [ast](https://github.com/YuanGongND/ast)
3. [ssast](https://github.com/YuanGongND/ssast)
4. [HTS-Audio-Transformer](https://github.com/RetroCirce/HTS-Audio-Transformer)
5. [AudioMAE](https://github.com/facebookresearch/AudioMAE)
6. [unilm/beats](https://github.com/microsoft/unilm/tree/master/beats)
7. [EAT](https://github.com/cwx-worst-one/EAT)


### 音频文本对

1. [LAION-AI/CLAP](https://github.com/LAION-AI/CLAP)
2. [WavCaps/retrieval](https://github.com/XinhaoMei/WavCaps/tree/master/retrieval)
3. [Cacophony](https://github.com/gzhu06/Cacophony)



# 新架构

介绍一些不同于MLP、RNN、Convolution和Transformer的架构
...
常用的离散方法为zero-order hold，具体是对于输入的离散信号，将其保持一段时间 $\Delta$，这里的 $\Delta$ 是一个可以学习的参数，直到下一个信号到来，假设在这段时间内输入和输出不变。

![](https://www.maartengrootendorst.com/images/posts/2024-21-02-mamba/24.svg)

在$t\in [k\Delta, (k+1)\Delta]$区间内，输入不变，即 $x(t) = x_k$，方程可以写成
$$
h'(t) = A{h(t)} + \bar B{x_k}
$$
这个是一阶线性常微分方程，假设$t_0$时的初值为 $h(t_0)$，则解为
$$
h(t) = {e^{A(t - {t_0})}}h({t_0}) + \int_{{t_0}}^t {{e^{A(t - \tau )}}Bx(\tau )} d\tau 
$$
代入$t_0=k\Delta$、$t=(k+1)\Delta$和$x(\tau)=x_k$，则有
$$
{h_{k + 1}} = {e^{A\Delta }}{h_k} + \int_{k\Delta }^{(k + 1)\Delta } {{e^{A((k + 1)\Delta  - \tau )}}} d\tau B{x_k}
$$
这样可以求解离散后的A和B
...
### Mamba 和 attention之间的联系

参考论文 [The Hidden Attention of Mamba Models - ACL Anthology](https://aclanthology.org/2025.acl-long.76/)

代码地址：[AmeenAli/HiddenMambaAttn: Official PyTorch Implementation of "The Hidden Attention of Mamba Models"](https://github.com/AmeenAli/HiddenMambaAttn)

对于Mamba，给定输入$\hat x'=(\hat x_1', \cdots, \hat x_L')$，输出$\hat y$可以表示为

![](images/ai/mamba_9.png)
S6可以表示为 $h_t = \bar A h_{t-1} + \bar B x_t, y_t=Ch_t$，几个矩阵的公式为（Sp为softplus）

![](images/ai/mamba_6-8.png)
S6会对每个通道的数据处理相互独立，所以仅考虑单个通道的输入，则有
![](images/ai/mamba_12.png)
用 $\tilde \alpha$ 来表示y关于x的系数，即 $y=\tilde \alpha x$。将之前几个矩阵的公式带入进去，可以得到
![](images/ai/mamba_17.png)
softplus可以用relu近似，得到
![](images/ai/mamba_19.png)
即最后可以简化为 $\tilde \alpha_{i,j} ≈ \tilde Q_i \tilde H_{i,j} \tilde K_j$

相比于Transformer中的Attention，mamba中加入了 $\tilde H_{i,j}$ 来控制不同位置之间的重要性。

一些研究利用这种性质提出了crossMamba，详见 [[2409.04803] Cross-attention Inspired Selective State Space Models for Target Sound Extraction](https://arxiv.org/abs/2409.04803)
...
论文地址：[2001.08361](https://arxiv.org/pdf/2001.08361)、[2203.15556](https://arxiv.org/pdf/2203.15556)

这篇论文讨论了一个问题：在给定计算量下，模型参数量和训练总token数（训练数据乘训练的轮次）如何权衡，假设N为模型参数，D为训练总token数，C为计算量，如果求出一个最优的N和D使得预训练损失$L(N, D)$最小

$$
{N_{opt}}(C),{D_{opt}}(C) = \mathop {\arg \min }\limits_{N,D\;s.t.\;{\rm{FLOPs}}(N,D) = C} L(N,D)
$$

论文中考虑了三种方法来计算最优N和D

1. 固定模型参数，修改训练总token数，最优为 ${N_{opt}} \propto {C^a}\quad {D_{opt}} \propto {C^b}$，a和b均取0.5
2. 在9种计算量设置下，修改模型参数，最优为${N_{opt}} \propto {C^a}\quad {D_{opt}} \propto {C^b}$，a=0.49，b=0.51
3. 针对前两个方法得到的数据进行建模，假设损失满足
$$
\hat L(N,D) = E + {A \over {{N^\alpha }}} + {B \over {{D^\beta }}}
$$
最优为
$$
{N_{opt}}(C) = G{\left( {{C \over 6}} \right)^a},{D_{opt}}(C) = {G^{ - 1}}{\left( {{C \over 6}} \right)^b}\quad G = {\left( {{{\alpha A} \over {\beta B}}} \right)^{{1 \over {\alpha  + \beta }}}},a = {\beta  \over {\alpha  + \beta }},b = {\alpha  \over {\alpha  + \beta }}
$$
其中a=0.46，b=0.51

在实际使用时，遵循以下步骤：
1. 先在不同数据规模训练一组大小不同的小模型，得到loss
2. 对得到的loss绘制log-log图，拟合幂律
$$
\eqalign{
  & L(N) = a{N^{ - \alpha }} + c  \cr 
  & L(D) = b{D^{ - \beta }} + c \cr} 
$$
3. 确定训练预算，假定每个token的训练成本为6N，则C=6ND
4. 根据上面的方法1，${N_{opt}} \propto {C^{0.5}}\quad {D_{opt}} \propto {C^{0.5}}$，给定C=6ND，Chinchilla给出的结论为：训练 tokens 数约等于模型参数数量的20倍
5. 根据$\alpha$和$\beta$判断通过模型规模还是数据规模来降低损失


## AnythingLLM

[Mintplex-Labs/anything-llm: The all-in-one Desktop & Docker AI application with built-in RAG, AI agents, No-code agent builder, MCP compatibility, and more.](https://github.com/Mintplex-Labs/anything-llm)

集成了langchain、ollama等工具
...
推理时可以在python中进行加载模型进行推理，也可以通过命令行进行推理


### 微调

[Reinforcement Learning from Human Feedback - vLLM](https://docs.vllm.ai/en/latest/training/rlhf/)

需要结合其它库如 trl 对大模型进行微调
...
用于微调大模型，还提供了一些数据集[Provided Datasets](https://github.com/hiyouga/LLaMA-Factory?tab=readme-ov-file#provided-datasets)
...
也可以使用[langchain_ollama](https://reference.langchain.com/python/integrations/langchain_ollama/)插件调用ollama下载到本地的模型，如下所示

```python
from langchain_ollama import ChatOllama

model = ChatOllama(model="gemma3:latest", validate_model_on_init=True,
    temperature=0.8,
    num_predict=256,
)
...
如果是huggingface上下载的模型，可以用 [langchain_huggingface](https://reference.langchain.com/python/integrations/langchain_huggingface/) 调用
...
langchain可以用来创建agents，agents将语言模型和工具结合起来，让大模型推理任务，决定用哪个工具。
...
agent中的model是agent的推理引擎，可以支持静态模型（直接读取模型），动态模型（可以切换模型）。
tools让agent可以操作资源（如查询天气，搜索资料），当然tools可以为空。
langchain可以设置[dynamic-system-prompt](https://docs.langchain.com/oss/python/langchain/agents#dynamic-system-prompt)，如根据用户角色来输入不同的系统提示词。

对于部分支持工具调用的模型，可以结构化输出
...
# qwen3:4b 支持工具调用
model = ChatOllama(model="qwen3:4b", validate_model_on_init=True,
    temperature=0.8,
    num_predict=256,
)
...
langchain支持三种访问大模型的方式：[invoke](https://docs.langchain.com/oss/python/langchain/models#invoke)（常规输入输出）、[stream](https://docs.langchain.com/oss/python/langchain/models#stream)（实时输出）和[batch](https://docs.langchain.com/oss/python/langchain/models#batch)（多输入多输出）

一些模型支持工具调用，具体可以参考下面这个例子
...
工具调用的函数使用 `@tool` 修饰，下面是一个基于agent的例子
...
还有一些高级特征，参见：[advanced-topics](https://docs.langchain.com/oss/python/langchain/models#advanced-topics)

### Messages

[Messages - Docs by LangChain](https://docs.langchain.com/oss/python/langchain/messages)

messages是对话的基本单元，包括Role、Content和Metadata

`langchain.messages`中提供了三种消息，分别为HumanMessage（代表用户输入）, AIMessage（代表模型生成的回答）和SystemMessage（针对模型的提示词），这些messages主要用来管理多轮对话、加入系统指令，如下所示
...
消息的内容还支持多模态信息，具体参考[message-content](https://docs.langchain.com/oss/python/langchain/messages#message-content)

### Short-term memory

[Short-term memory - Docs by LangChain](https://docs.langchain.com/oss/python/langchain/short-term-memory)

short-term memory让模型能够记忆之前的交互信息，为了给agent添加一个short-term memory，需要在创建agent时指定一个checkpointer，如下所示
...
print(response2)   # 加入InMemorySaver，可以让模型正确回答这个问题
```

如果需要在生产中使用，参考：[in-production](https://docs.langchain.com/oss/python/langchain/short-term-memory#in-production)

不同用户应该对应不同的memory，可以参考[customizing-agent-memory](https://docs.langchain.com/oss/python/langchain/short-term-memory#customizing-agent-memory)通过`state_schema` 来管理用户。

如果保存了short-term memory，长对话可能会超过LLM的上下文长度，为了解决这个问题，有四个常见的解决方法，简单来说就是删除一些之前的消息，或者总结之前的消息等，详见[common-patterns](https://docs.langchain.com/oss/python/langchain/short-term-memory#common-patterns)

langchain中提供了一些方法来访问记忆的内容，详见[access-memory](https://docs.langchain.com/oss/python/langchain/short-term-memory#access-memory)。

### Middleware

[Middleware - Docs by LangChain](https://docs.langchain.com/oss/python/langchain/middleware)

通过暴露一些hook控制agent中的过程，可以控制的过程如下所示
![](https://mintcdn.com/langchain-5e9cc07a/RAP6mjwE5G00xYsA/oss/images/middleware_final.png?w=560&fit=max&auto=format&n=RAP6mjwE5G00xYsA&q=85&s=41b7dd647447978ff776edafe5f42499)
...
MCP是一个用来标准化应用如何向LLM提供工具和上下文的开源协议，需要使用langchain-mcp-adapters库来定义。mcp支持几种机制：stdio（开辟一个子线程来执行任务，然后通过标准输入输出来获取信息）、Streamable HTTP（发送一个http请求）和Server-Sent Events（类似Streamable HTTP，但是针对实时流式通信）
...
Retrieval可以通过获取相关的外部知识来解决LLMs无法获取完整的知识的问题，是Retrieval-Augmented Generation的基础。下面是一个RAG的流程
...
下面介绍一个具体的例子[Build a semantic search engine with LangChain - Docs by LangChain](https://docs.langchain.com/oss/python/langchain/knowledge-base)
先安装库langchain-community和pypdf，为了跟踪agent的内部过程，可以使用[langSmith](https://smith.langchain.com/)。下载一个[示例pdf](https://github.com/langchain-ai/langchain/blob/v0.3/docs/docs/example_data/nke-10k-2023.pdf)，首先加载pdf，然后再切分文本，将文本送入模型编码为embeddings（不同模型使用的embedding方法不同），embedding一般会存储在向量数据库（如chroma）中，向量数据库会提供一些方法进行相似度搜索
...
all_splits = text_splitter.split_documents(docs)  # split document
...
> 除了pdf，langchain中还有一些插件如[Microsoft](https://docs.langchain.com/oss/python/integrations/providers/microsoft)，可以用于解析word，excel等

## Ollama

[Ollama](https://ollama.com/)

可以用于部署大模型，有多种大模型可选，参考[Ollama Search](https://ollama.com/search)，其中一些参数较少的大模型可以在本地单个GPU运行

### 安装

Windows：在[Download Ollama on Windows](https://ollama.com/download)中下载安装包，默认的安装路径在C盘，可以参考[Windows - Ollama](https://docs.ollama.com/windows)修改安装路径
```powershell
OllamaSetup.exe /DIR="d:\some\location"
```

Linux：建议直接参考[Linux - Ollama](https://docs.ollama.com/linux)中的Manual install进行安装


### 运行

在Windows下直接运行应用即可，如果想要以命令行的形式运行，可以参考Linux下的运行方式

在Linux下，首先启动ollama的本地服务器

```bash
ollama serve
```

然后可以在另外一个终端中输入命令 `ollama -v` 来判断是否正在运行
...
ollama服务器默认会监听 `http://localhost:11434`，所以可以通过下面的命令来查看ollama的版本
...
如果希望在局域网中访问ollama服务，可以在一台机器中通过下面的指令运行ollama服务
...
然后在这台机器中另外一个终端中运行大模型即可

只需要获得运行ollama服务的机器的ip地址（假设为 192.168.110.31）即可访问服务，如下所示

```bash
curl http://192.168.110.31:11434/api/version
```


### Go+ollama

使用go语言访问ollama服务，并使用ollama提供的搜索功能

使用ollama的搜索功能[Web search - Ollama](https://docs.ollama.com/capabilities/web-search)需要申请api key，搜索功能可以通过curl命令实现，go 中直接通过 `net/http` 包实现

聊天功能[Structured Outputs - Ollama](https://docs.ollama.com/capabilities/structured-outputs) 也可以通过 `net/http`包实现

代码地址为：[test_ollama.go](https://gist.github.com/Xiang-M-J/bf8ca0a99c336865f8de00fc603b4f0b)
...
上面的go+ollama需要写很多代码，而Genkit这个库可以简化这个过程
...
[Eino: 概述 | CloudWeGo](https://www.cloudwego.io/zh/docs/eino/overview/)
